-- ============================================================================
-- 070: 生产管理模块增强 — 核心流程表（原编号 054，因与 054_add_work_order_cumulative_fields 重复而顺延）
-- 功能: 工单状态增强、领料/退料/报工/完工入库表
-- ============================================================================

-- 1. 扩展 prod_work_order 表，增加工序和成本字段（使用 IF NOT EXISTS 保证可重复执行）
ALTER TABLE `prod_work_order`
    ADD COLUMN IF NOT EXISTS `order_type` tinyint DEFAULT 0 COMMENT '工单类型: 0-正常 1-打样工单 2-返工' AFTER `product_name`,
    ADD COLUMN IF NOT EXISTS `approved_at` datetime DEFAULT NULL COMMENT '审核时间' AFTER `total_material_cost`,
    ADD COLUMN IF NOT EXISTS `approved_by` bigint unsigned DEFAULT NULL COMMENT '审核人ID' AFTER `approved_at`,
    ADD COLUMN IF NOT EXISTS `cancelled_at` datetime DEFAULT NULL COMMENT '作废时间' AFTER `approved_by`,
    ADD COLUMN IF NOT EXISTS `cancelled_by` bigint unsigned DEFAULT NULL COMMENT '作废人ID' AFTER `cancelled_at`,
    ADD COLUMN IF NOT EXISTS `cancelled_reason` varchar(500) DEFAULT NULL COMMENT '作废原因' AFTER `cancelled_by`,
    ADD COLUMN IF NOT EXISTS `total_labor_cost` decimal(18,4) DEFAULT 0 COMMENT '人工成本合计' AFTER `total_material_cost`,
    ADD COLUMN IF NOT EXISTS `total_tool_cost` decimal(18,4) DEFAULT 0 COMMENT '工装分摊成本' AFTER `total_labor_cost`,
    ADD COLUMN IF NOT EXISTS `total_overhead_cost` decimal(18,4) DEFAULT 0 COMMENT '制造费用' AFTER `total_tool_cost`,
    ADD COLUMN IF NOT EXISTS `unit_cost` decimal(18,4) DEFAULT 0 COMMENT '单位成本' AFTER `total_overhead_cost`;
