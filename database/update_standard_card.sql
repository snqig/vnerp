-- ========================================================
-- 标准卡表结构更新（print / input 页面专用 schema）
-- 根据 print 页面和 input 页面需求重新设计
-- ========================================================
--
-- ⚠️ 安全警告（P1-17 修复）：
-- 1. 原脚本使用 `DROP TABLE IF EXISTS prd_standard_card` 会清空所有已有数据，
--    已改为 `CREATE TABLE IF NOT EXISTS`，避免误执行导致生产数据丢失。
-- 2. 本文件与 standard_card_migration.sql 都定义了 `prd_standard_card`，
--    但两者字段结构互不兼容（本文件为 print 取向的 78 字段单表设计；
--    standard_card_migration.sql 为 code/is_current 的模块化设计，且被
--    MysqlStandardCardRepository 使用）。两张表名冲突属于架构层面的技术债，
--    需由团队决定唯一权威 schema 后再统一代码与 SQL，禁止在本脚本中 DROP。
-- 3. 如需在开发环境重置该表，请手动执行 TRUNCATE，而非 DROP 整表。
-- ========================================================

-- 创建标准卡表（若不存在则创建，绝不 DROP 已有数据）
...
-- 插入测试数据（INSERT IGNORE：uk_card_no 唯一约束保证重复执行不会报错/重复）
