...
| 能力 | 现状 | 说明 |
| ---- | ---- | ---- |
| 版本对比 `FormulaCompareService` | 🚧 | 算法待实现，API `/version/compare` 路由未建 |
| 成本预览 `/version/:id/preview-cost` | 🚧 | 成本快照字段已在版本表，但草稿态实时预览接口未建 |
| 成本重算 `/version/:id/recalculate-cost` | 🚧 | 手动重算接口未建 |
| 明细更新 `/version/:id/items` | 🚧 | 当前通过版本 PUT 整体更新，未拆分独立明细路由 |
| `FormulaVersionActivatedEvent` 事件 | 🚧 | 生效流程已实现，但事件总线联动下游（工单引用配方携带快照成本）未打通 |
...
| 2. 领域层核心实现 | 🚧 | 应用服务已实现并跑通业务流程；聚合根 `InkFormulaVersion` / 值对象 / 仓储接口的纯领域层封装待补 |
| 3. 应用层与基础设施层 | 🚧 | Service 已实现；`MysqlFormulaVersionRepository` 独立仓储未抽离（当前 Service 内联 mysql2 查询） |
| 4. API 接口开发 | 🚧 | 色号/版本 CRUD + 生效/作废/复用已实现；对比/成本预览/重算/明细独立路由未建 |
| 5. 前端页面开发 | ✅ | 色号管理 + 版本管理页已实现；版本对比弹窗、成本实时预览待补 |
