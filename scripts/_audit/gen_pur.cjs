// pur_ 域补全生成器：生成 9 张未建模核心 pur_* 表到 _gen_pur.ts
// 复用 gen_sys.cjs / gen_warehouse_missing.cjs 的 colBuilder/fmtDefault/parseType 与拓扑排序逻辑。
// 内部 FK（pur_* → 已建模/待生成 pur_*）实写；跨域 FK 注释保留。
const fs = require('fs');
...
// 6 张已建模 pur_*（procurement.ts）
const MODELED = new Set([
  'pur_purchase_order','pur_purchase_order_line','pur_purchase_return',
  'pur_purchase_return_line','pur_purchase_reconciliation','pur_supplier',
]);
const MODELED_EXPORTS = {
  pur_purchase_order:'purPurchaseOrder', pur_purchase_order_line:'purPurchaseOrderLine',
  pur_purchase_return:'purPurchaseReturn', pur_purchase_return_line:'purPurchaseReturnLine',
  pur_purchase_reconciliation:'purPurchaseReconciliation', pur_supplier:'purSupplier',
};

const TARGET = [
  'pur_order_deprecated',
  'pur_order_detail_deprecated',
  'pur_purchase_reconciliation_writeoff',
  'pur_receipt_deprecated',
  'pur_receipt_detail_deprecated',
  'pur_request',
  'pur_request_detail',
  'pur_request_item',
...
const exportName = (t) => 'pur' + camel(t.replace(/^pur_/, '')).replace(/^\w/, c => c.toUpperCase());
...
(async () => {
  ...
  const knownPur = new Set([...MODELED, ...TARGET]);
  ...
  const lines = [];
  lines.push("// AUTO-GENERATED by scripts/_audit/gen_pur.cjs (2026-08-26)");
  lines.push("// 来源: live information_schema. 建模 9 张核心 pur_* 缺失表（含 5 张 *_deprecated 废弃表，真实存在须建模以免护栏误判 DROP）。");
  lines.push("// 跨域 FK / 自引用 FK 以注释保留；内部 pur_* FK 实写。");
  ...
  if (needImports.size) {
    lines.push(`import { ${[...needImports].join(', ')} } from './procurement';`);
  }
  lines.push('');
  ...
  const target = path.resolve(__dirname, '..', '..', 'src', 'lib', 'db', 'schemas', '_gen_pur.ts');
