...
async function fixRoleTable() {
  ...
  try {
    ...
    // 2. 检查是否有 permissions 字段
    if (!columnNames.includes('permissions')) {
      console.log('\n❌ 缺少 permissions 字段，正在添加...');
      await connection.execute(
        `ALTER TABLE sys_role 
         ADD COLUMN permissions JSON NULL COMMENT '权限列表' 
         AFTER status`
      );
      console.log('✅ permissions 字段添加成功');
    } else {
      console.log('\n✅ permissions 字段已存在');
    }
