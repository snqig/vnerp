 ...
 * 把命名空间对象展平为「点号路径」集合。
 *
 * next-intl 的 key 支持嵌套点号寻址：messages 里 `Monitoring.consistency.title`
 * 既可用 `useTranslations('Monitoring')` + `t('consistency.title')` 访问，
 * 也可用 `useTranslations('Monitoring.consistency')` + `t('title')`。
 * 因此比对前必须展平，否则一切嵌套 key 都会被误判为「缺失」
 * （这是本校验器早期版本的主要假阳性来源）。
 */
function flattenInto(node, prefix, acc) {
  for (const [k, v] of Object.entries(node || {})) {
    const path = prefix ? `${prefix}.${k}` : k;
    if (v !== null && typeof v === 'object' && !Array.isArray(v)) {
      flattenInto(v, path, acc);
    } else {
      acc.add(path);
    }
  }
...
function loadMessages() {
  ...
  for (const ns of Object.keys(data)) {
    ...
    flattenInto(data[ns], '', namespaces[ns]);
  }
  return { data, namespaces };
}

// ---------- 解析单个文件 ----------
// 绑定形态：必须与 verify-codemod-consistency.mjs 的扫描块保持一致，
// 否则门禁（pre-commit）与报表口径会漂移。原正则要求 `=` 后紧跟 useTranslations，
// 漏掉服务端主流写法 `const ts = await getTranslations('Common')` 与
// 对象形态 `await getTranslations({ locale, namespace: 'Common' })`。
...
function analyzeFile(absPath, relPath, namespaces, globalKeyNs) {
  ...
  // 1. 找出本文件所有别名 -> 命名空间 绑定（字符串形态 + 对象形态）
  const aliasToNs = {};
...
function main() {
  ...
  // 孤立 key（代码中从未引用）
  // 注意：必须遍历展平后的 namespaces（点号路径），否则嵌套节点会被当成
  // 独立 key（如 `Monitoring.consistency`）而全部误报为孤立。
  let unused = [];
  if (CHECK_UNUSED) {
