...
// 与库存批次/汇总同源，保证"生成数据即启用流水账、账实可追"。
async function appendInventoryTxn(conn, p) {
  const transNo = `TXN-${p.transType}-${p.sourceType}-${p.sourceId}-${p.batchNo}-${Math.floor(
    Math.random() * 1e6
  )}`;
  await conn.execute(
    `INSERT INTO inv_inventory_transaction
      (trans_no, trans_type, source_type, source_id, material_id, material_code, batch_no,
       warehouse_id, quantity, unit_price, total_amount, reference_no, remark, create_time)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
    [
      transNo, p.transType, p.sourceType, p.sourceId, p.materialId, p.materialCode || null,
      p.batchNo, p.warehouseId, p.quantity, p.unitPrice, p.totalAmount,
      p.referenceNo || null, p.remark || null,
    ]
  );
}
...
async function main() {
  ...
  // 流水账：入库 A 写入 inv_inventory_transaction
  await appendInventoryTxn(conn, {
    transType: 'in', sourceType: 'inbound_order', sourceId: inb1Id,
    materialId: matBlackInk.id, materialCode: matBlackInk.material_code,
    batchNo: 'BATCH-20260701-A', warehouseId: whRaw.id,
    quantity: 1500, unitPrice: 85, totalAmount: 1500 * 85,
    referenceNo: 'INB-2026-001', remark: '测试数据-采购入库-黑色油墨',
  });
  ...
  // 流水账：入库 B 写入 inv_inventory_transaction
  await appendInventoryTxn(conn, {
    transType: 'in', sourceType: 'inbound_order', sourceId: inb2Id,
    materialId: matWhiteInk.id, materialCode: matWhiteInk.material_code,
    batchNo: 'BATCH-20260701-B', warehouseId: whRaw.id,
    quantity: 1000, unitPrice: whiteInkBasePrice, totalAmount: 1000 * whiteInkBasePrice,
    referenceNo: 'INB-2026-002', remark: '测试数据-采购入库-白色油墨',
  });
  ...
  // 流水账：入库 C 写入 inv_inventory_transaction
  await appendInventoryTxn(conn, {
    transType: 'in', sourceType: 'inbound_order', sourceId: inb3Id,
    materialId: matScreen.id, materialCode: matScreen.material_code,
    batchNo: 'BATCH-20260701-C', warehouseId: whRaw.id,
    quantity: 500, unitPrice: 50, totalAmount: 500 * 50,
    referenceNo: 'INB-2026-003', remark: '测试数据-采购入库-网版',
  });
  ...
  // 流水账：领料出库 A 写入 inv_inventory_transaction
  await appendInventoryTxn(conn, {
    transType: 'out', sourceType: 'material_issue', sourceId: issueId,
    materialId: matBlackInk.id, materialCode: matBlackInk.material_code,
    batchNo: 'BATCH-20260701-A', warehouseId: whRaw.id,
    quantity: PICK_BLACK_INK_QTY, unitPrice: 85, totalAmount: PICK_BLACK_INK_QTY * 85,
    referenceNo: 'MR-2026-001', remark: '测试数据-领料出库-黑色油墨',
  });
  ...
  // 流水账：领料出库 B 写入 inv_inventory_transaction
  await appendInventoryTxn(conn, {
    transType: 'out', sourceType: 'material_issue', sourceId: issueId,
    materialId: matWhiteInk.id, materialCode: matWhiteInk.material_code,
    batchNo: 'BATCH-20260701-B', warehouseId: whRaw.id,
    quantity: PICK_WHITE_INK_QTY, unitPrice: whiteInkBasePrice, totalAmount: PICK_WHITE_INK_QTY * whiteInkBasePrice,
    referenceNo: 'MR-2026-001', remark: '测试数据-领料出库-白色油墨',
  });
  ...
  // 流水账：领料出库 C 写入 inv_inventory_transaction
  await appendInventoryTxn(conn, {
    transType: 'out', sourceType: 'material_issue', sourceId: issueId,
    materialId: matScreen.id, materialCode: matScreen.material_code,
    batchNo: 'BATCH-20260701-C', warehouseId: whRaw.id,
    quantity: PICK_SCREEN_QTY, unitPrice: 50, totalAmount: PICK_SCREEN_QTY * 50,
    referenceNo: 'MR-2026-001', remark: '测试数据-领料出库-网版',
  });
  ...
  // ═══════════════════════════════════════════════════════
  // 派生重算库存汇总表 inv_inventory（由 inv_inventory_batch 汇总）
  // 保证"生成数据即账实一致"：汇总 = SUM(批次)，与运行期 recomputeInventorySummary 同口径
  // ═══════════════════════════════════════════════════════
  console.log('\n━━━ 派生重算库存汇总表 inv_inventory（由 inv_inventory_batch）━━━');
  const [recomputeResult] = await conn.execute(
  ...
  console.log(`  [ok] inv_inventory 派生重算插入 ${recomputeResult.affectedRows} 行`);
