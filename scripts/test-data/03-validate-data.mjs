...
async function main() {
  ...
  // 11d. 流水账与汇总一致性（启用流水账后回归校验）
  const summaryRows = await query(conn,
    `SELECT i.material_id, m.material_name, i.quantity, i.available_qty, i.warehouse_id
     FROM inv_inventory i JOIN inv_material m ON m.id = i.material_id
     WHERE i.deleted = 0 AND i.warehouse_id = (SELECT id FROM inv_warehouse WHERE warehouse_code = 'WH001' AND deleted = 0)
     ORDER BY i.material_id`);
  check('库存汇总表 inv_inventory 有 3 行', summaryRows.length === 3, `实际=${summaryRows.length}`);

  let summaryOk = true;
  for (const s of summaryRows) {
    const [agg] = await query(conn,
      `SELECT SUM(quantity) q, SUM(available_qty) a FROM inv_inventory_batch
       WHERE material_id = ? AND warehouse_id = ? AND deleted = 0`,
      [s.material_id, s.warehouse_id]);
    if (Math.abs(Number(s.quantity) - Number(agg.q)) > 0.001) summaryOk = false;
    if (Math.abs(Number(s.available_qty) - Number(agg.a)) > 0.001) summaryOk = false;
  }
  check('汇总表 数量/可用量 = 批次 SUM（账实一致）', summaryOk);

  const txnCounts = await query(conn,
    `SELECT trans_type, COUNT(*) cnt FROM inv_inventory_transaction GROUP BY trans_type`);
  const cntMap = {};
  txnCounts.forEach(r => { cntMap[r.trans_type] = Number(r.cnt); });
  check('流水账 入库(in) = 3 笔', cntMap['in'] === 3, `实际=${cntMap['in']}`);
  check('流水账 出库(out) = 3 笔', cntMap['out'] === 3, `实际=${cntMap['out']}`);

  const txnA = await query(conn,
    `SELECT trans_type, quantity FROM inv_inventory_transaction WHERE batch_no = 'BATCH-20260701-A' ORDER BY trans_type`);
  check('批次A 入库流水 = 1500', txnA.some(t => t.trans_type === 'in' && Number(t.quantity) === 1500));
  check('批次A 出库流水 = 300', txnA.some(t => t.trans_type === 'out' && Number(t.quantity) === 300));
