...
export default function FinancePage() {
  ...
  const fetchSummary = useCallback(async () => {
    try {
      ...
      if (data.success && data.data) {
        setSummary({
          receivable: {
            total_amount: data.data.receivable.total_amount,
            total_received: data.data.receivable.total_received,
            total_balance: data.data.receivable.total_balance,
            overdue_balance: data.data.receivable.overdue_balance,
          },
          payable: {
            total_amount: data.data.payable.total_amount,
            total_paid: data.data.payable.total_paid,
            total_balance: data.data.payable.total_balance,
            overdue_balance: data.data.payable.overdue_balance,
          },
        });
      }
    } catch (_e) {
  ...
  const handleViewDetail = async (id: number, type: 'receivable' | 'payable') => {
    // 记录已在应收/应付列表中加载（含 receivable_no/customer_name/amount/balance 等），
    // 直接按 id 从列表取单条即可；原先请求 /api/finance/receivables?id= 返回的是整个列表信封，
    // 导致详情弹窗读到的字段全是 undefined。关联的收/付款子记录（detailData.receipts/payments）
    // 当前列表未携带，弹窗中相应小节将不展示（不影响主字段）。
    const list = type === 'receivable' ? receivables : payables;
    const record = list.find((x: Loose) => x.id === id);
    if (record) {
