...
export default function EditBOMPage() {
  ...
  useEffect(() => {
    const fetchBOM = async () => {
      try {
        setFetching(true);
        const res = await authFetch(`/api/orders/bom/${bomId}`);
        ...
        if (data.success) {
          ...
          setFormData({
            product_id: header.product_id || 0,
          ...
          setLines(
            apiLines.map((line: Loose) => ({
              ...
              material_type: line.material_type || 'RAW',
              is_key_material: !!line.is_key_material,
              position_no: line.position_no || '',
              process_seq: line.process_seq || null,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        toast({
          title: tc('success'),
          description: t('bomUpdateSuccess') || t('bomCreateSuccess'),
        });
        router.push('/orders/bom');
      } else {
        toast({
          title: tc('error'),
          description: data.message || t('bomUpdateFailed') || t('bomCreateFailed'),
          variant: 'destructive',
        });
      }
    } catch {
      toast({
        title: tc('error'),
        description: t('bomUpdateFailed') || t('bomCreateFailed'),
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  if (fetching) {
    return (
      <MainLayout title={t('editBom') || t('createBom')}>
