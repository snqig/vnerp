...
import { ArrowLeft, Save, Building2, Upload, FileText, X } from 'lucide-react';
...
export default function NewCustomerPage() {
  ...
  const handleUploadLicense = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      fd.append('file', file);
      const res = await authFetch('/api/upload/customer-license', {
        method: 'POST',
        body: fd,
      });
    ...
    try {
      ...
      const response = await authFetch('/api/customers', {
        ...
        body: JSON.stringify({
          ...
          license_file_url: licenseFile?.url || '',
        }),
      });
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                {licenseFile ? (
                  ...
                  <label
                    className="flex items-center justify-center gap-2 rounded-md border border-dashed border-input bg-muted/20 px-3 py-2 cursor-pointer hover:bg-muted/40 transition-colors"
                  >
