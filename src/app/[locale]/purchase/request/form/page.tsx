  ...
  const handleSave = async (submitStatus: number = 0) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // 后端按系统设置 category.require_on_business 返回的"物料未归类"提示，
        // 不能被成功 toast 吞掉——采购统计与成本归集都依赖物料分类
        const uncategorized = result.data?.uncategorizedMaterials as
        ...
        if (uncategorized && uncategorized.length > 0) {
          toast({
            ...
            description: `${uncategorized
              ...
              .join('、')}${
              uncategorized.length > 3 ? ` 等 ${uncategorized.length} 项` : ''
            }，请到「基础数据 → 物料分类」补齐`,
            variant: 'destructive',
          });
        }
