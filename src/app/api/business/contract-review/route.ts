    ...
    const result: Loose = await execute(
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        ...
        material_availability || null,
        engineering_feasibility || null,
        biz_opinion || null,
        eng_opinion || null,
        quality_opinion || null,
        prod_opinion || null,
        purchase_opinion || null,
        review_date || null,
        remark || null,
      ]
    );
...
export const PUT = withPermission(
  async (request: NextRequest, _userInfo) => {
    ...
    if (quality_requirement !== undefined) {
      fields.push('quality_requirement = ?');
      values.push(quality_requirement);
    }
    if (production_capacity !== undefined) {
      fields.push('production_capacity = ?');
      values.push(production_capacity);
    }
    if (material_availability !== undefined) {
      fields.push('material_availability = ?');
      values.push(material_availability);
    }
    if (engineering_feasibility !== undefined) {
      fields.push('engineering_feasibility = ?');
      values.push(engineering_feasibility);
    }
    if (biz_opinion !== undefined) {
      fields.push('biz_opinion = ?');
      values.push(biz_opinion);
    }
    if (eng_opinion !== undefined) {
      fields.push('eng_opinion = ?');
      values.push(eng_opinion);
    }
    if (quality_opinion !== undefined) {
      fields.push('quality_opinion = ?');
      values.push(quality_opinion);
    }
    if (prod_opinion !== undefined) {
      fields.push('prod_opinion = ?');
      values.push(prod_opinion);
    }
    if (purchase_opinion !== undefined) {
      fields.push('purchase_opinion = ?');
      values.push(purchase_opinion);
    }
