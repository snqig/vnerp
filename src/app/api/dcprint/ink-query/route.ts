...
async function queryFormulaTrace(batchNo: string) {
  ...
  if (dispatchRows.length > 0) {
    ...
    trace.raw_inks = dispatchItems.map((item: DbRow) => ({
      ink_name: item.ink_name,
      ink_code: item.ink_code,
      brand: item.brand,
...
async function queryProcessGuide(batchNo: string) {
  ...
  if (dispatchRows.length > 0 && dispatchRows[0].workorder_id) {
    const woRows = await query('SELECT * FROM prod_work_order WHERE id = ? AND deleted = 0', [
    ...
    if (woRows.length > 0) {
      ...
      if (wo.process_card_id) {
        const pcRows = await query('SELECT * FROM prd_process_card WHERE id = ? AND deleted = 0', [
          wo.process_card_id,
        ]);
        if (pcRows.length > 0) {
          guide.process_card = {
