...
export const POST = withPermission(
  ...
  const ts = await getTranslations('Common');
    ...
    if (!card_data) return errorResponse(ts('k_9g1479'), 400, 400);
...
export const PUT = withPermission(
  ...
  const ts = await getTranslations('Common');
    ...
    if (action === 'approve') {
      ...
      if (!card_id || !version) return errorResponse(ts('k_owgh6n'), 400, 400);
      await transaction(async (conn) => {
        await approveCardVersion(conn, card_id, version, approver_name || 'system');
      });
      return successResponse({ message: ts('k_1x54mqe') });
    }

    const { card_id, updates, change_description } = body;
    if (!card_id || !updates) return errorResponse(ts('k_ypecns'), 400, 400);
...
export const GET = withPermission(async (request: NextRequest, _userInfo) => {
  ...
  if (action === 'versions') {
    ...
    if (!cardId) return errorResponse(ts('k_d5cucu'), 400, 400);
  ...
  if (action === 'compare') {
    ...
    if (!v1 || !v2) return errorResponse(ts('k_14yac0k'), 400, 400);
  ...
  const total = await query(`SELECT COUNT(*) as count FROM prd_process_card ${where}`, params);
