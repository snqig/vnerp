...
export const GET = withErrorHandler(async (request: NextRequest) => {
  ...
  if (type === 'plan') {
    ...
    if (maintenanceType) {
      ...
      params.push(Number(maintenanceType));
    }
    if (status) {
      where += ' AND p.status = ?';
    ...
    const rows: any = await query(
      'SELECT p.*, e.equipment_code, e.equipment_name FROM eqp_maintenance_plan p LEFT JOIN eqp_equipment e ON p.equipment_id = e.id ' +
        where +
        ' ORDER BY p.plan_date ASC LIMIT ? OFFSET ?',
  ...
  const total = totalRows[0]?.total || 0;
  const rows: any = await query(
    'SELECT r.*, e.equipment_code, e.equipment_name FROM eqp_maintenance_record r LEFT JOIN eqp_equipment e ON r.equipment_id = e.id ' +
      where +
...
export const PUT = withErrorHandler(async (request: NextRequest) => {
  ...
  if (type === 'plan') {
    ...
    if (status !== undefined) {
      await execute('UPDATE eqp_maintenance_plan SET status = ? WHERE id = ? AND deleted = 0', [
        status,
        id,
      ]);
    }
    if (maintenance_type !== undefined) {
      await execute(
        'UPDATE eqp_maintenance_plan SET maintenance_type = ? WHERE id = ? AND deleted = 0',
        [maintenance_type, id]
      );
    }
    if (cycle_type !== undefined) {
      await execute('UPDATE eqp_maintenance_plan SET cycle_type = ? WHERE id = ? AND deleted = 0', [
        cycle_type,
        id,
      ]);
    }
    if (cycle_value !== undefined) {
      await execute(
        'UPDATE eqp_maintenance_plan SET cycle_value = ? WHERE id = ? AND deleted = 0',
        [cycle_value, id]
      );
    }
    if (plan_date !== undefined) {
      await execute('UPDATE eqp_maintenance_plan SET plan_date = ? WHERE id = ? AND deleted = 0', [
        plan_date,
        id,
      ]);
    }
    if (content !== undefined) {
      await execute('UPDATE eqp_maintenance_plan SET content = ? WHERE id = ? AND deleted = 0', [
        content,
        id,
      ]);
    }
    if (remark !== undefined) {
      await execute('UPDATE eqp_maintenance_plan SET remark = ? WHERE id = ? AND deleted = 0', [
        remark,
        id,
      ]);
    }
  ...
  if (maintenance_content !== undefined) {
    await execute(
      'UPDATE eqp_maintenance_record SET maintenance_content = ? WHERE id = ? AND deleted = 0',
      [maintenance_content, id]
    );
  }
  if (end_time !== undefined) {
    await execute('UPDATE eqp_maintenance_record SET end_time = ? WHERE id = ? AND deleted = 0', [
      end_time,
      id,
    ]);
  }
  if (downtime_hours !== undefined) {
    await execute(
      'UPDATE eqp_maintenance_record SET downtime_hours = ? WHERE id = ? AND deleted = 0',
      [downtime_hours, id]
    );
  }
  if (cost !== undefined) {
    await execute('UPDATE eqp_maintenance_record SET cost = ? WHERE id = ? AND deleted = 0', [
      cost,
      id,
    ]);
  }
  if (maintResult !== undefined) {
    await execute('UPDATE eqp_maintenance_record SET result = ? WHERE id = ? AND deleted = 0', [
      maintResult,
      id,
    ]);
  }
  if (remark !== undefined) {
    await execute('UPDATE eqp_maintenance_record SET remark = ? WHERE id = ? AND deleted = 0', [
      remark,
      id,
    ]);
  }
