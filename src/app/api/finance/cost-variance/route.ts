...
export const GET = withPermission(async (request: NextRequest) => {
  ...
  const workOrders = await query(
    ...
    LEFT JOIN sal_order_item soi ON wo.order_id = soi.order_id
    ${where}
  ...
  for (const wo of workOrders) {
    const materialCostRows = await query(
      ...
      LEFT JOIN inv_inventory_transaction tr ON tr.source_no = ? AND tr.trans_type = 'out'
      WHERE wr.work_order_id = ? AND wr.deleted = 0
    `,
      [wo.id, wo.work_order_no, wo.id]
    );

    const laborCostRows = await query(
      `
      SELECT
