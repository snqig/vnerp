...
export const POST = withErrorHandler(async (request: NextRequest) => {
  const result = await transaction(async (conn) => {
    ...
    const clearTables = [
      'inv_trace_detail', 'inv_trace_record', 'inv_scan_log',
      'prd_product_label', 'inv_production_inbound_item', 'inv_production_inbound',
      'prd_work_report', 'prd_material_issue_item', 'prd_material_issue',
    ...
    const [userRows]: any = await conn.execute('SELECT id, username, real_name FROM sys_user WHERE deleted=0 ORDER BY id');
    ...
    const whCodes = ['Y01', 'B01', 'C01', 'A01', 'M01', 'H01', 'L01', 'Z01', 'R01', 'S01', 'P01', 'SP01', 'SM01', 'T01', 'O01'];
    ...
    for (let i = 0; i < whNames.length; i++) {
      ...
      await conn.execute(
        ...
        [whCodes[i], whNames[i], (i % 10) + 1, '广东省', '东莞', `广东省东莞市工业园区${i + 1}号`, whUser.id, `0769-${randomInt(22000000, 22999999)}`, 1, null]
      );
    ...
    // ===== 重建基础数据：物料 =====
    const matTypes = [
      { prefix: 'PET', name: 'PET薄膜', unit: '张', cat_type: 1, specs: ['0.1mm×500mm', '0.125mm×600mm', '0.188mm×700mm', '0.25mm×800mm'], price_range: [0.5, 5.0] },
      ...
      { prefix: 'AUX', name: '辅助材料', unit: '个', cat_type: 3, specs: ['网框-铝合金', '网纱-77T', '刮胶-65度', '保护膜-50μm'], price_range: [5, 200] },
    ...
    for (let i = 1; i <= 50; i++) {
      ...
      await conn.execute(
        ...
        [code, `${mt.name}${pad(i, 3)}`, spec, catType?.id || null, mt.cat_type, mt.unit, randomItem(['3M', '杜邦', '东洋', '精工', '国产']), randomInt(100, 1000), randomInt(5000, 50000), randomInt(50, 500), purchasePrice, salePrice, Math.round(purchasePrice * 1.05 * 100) / 100, whMap['Y01'], mt.cat_type <= 2 ? 1 : 0, 1]
      );
      const [matRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
      matMap[code] = matRow[0].id;
      matInfoMap[code] = { id: matRow[0].id, material_code: code, material_name: `${mt.name}${pad(i, 3)}`, specification: spec, unit: mt.unit, purchase_price: purchasePrice, sale_price: salePrice, cat_type: mt.cat_type };
    }
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      await conn.execute(
        ...
        [`CUS-${pad(i, 5)}`, `东莞${industry}有限公司`, `${industry.substring(0, 2)}`, (i % 3) + 1, industry, randomItem(['大型', '中型', '小型']), randomItem(['AAA', 'AA', 'A']), '广东', '东莞', `广东省东莞市工业区${i}号`, `联系人${i}`, `1${randomInt(3000000000, 3999999999)}`, `c${i}@example.com`, `91440100MA5C${pad(i, 6)}`, randomItem(['工商银行', '建设银行']), `6222${String(randomInt(1000000000000, 9999999999999))}`, adminUser.id, (i % 5) + 1, 1]
      );
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      await conn.execute(
        ...
        [`SUP-${pad(i, 5)}`, `东莞${sType}供应商${i}有限公司`, `${sType.substring(0, 2)}`, (i % 3) + 1, '广东', '东莞', `广东省东莞市供应商路${i}号`, `供应商联系人${i}`, `1${randomInt(3800000000, 3999999999)}`, `s${i}@example.com`, `91440100MA5D${pad(i, 6)}`, randomItem(['工商银行', '建设银行']), `6228${String(randomInt(1000000000000, 9999999999999))}`, randomItem(['AAA', 'AA', 'A']), 1, randomItem(['月结30天', '月结60天']), randomItem(['月结30天', '货到付款']), 1]
      );
    ...
    const warehouses: any[] = Object.entries(whMap).map(([code, id]) => ({ id, warehouse_code: code }));
    ...
    const customers = cusIds.map((id, i) => ({ id, customer_name: `东莞${industries[i % industries.length]}有限公司` }));
    const suppliers = supIds.map((id, i) => ({ id, supplier_name: `东莞${randomItem(['原材料', '油墨', '辅料'])}供应商${i + 1}有限公司` }));

    const productNames = ['空调面板标签', '洗衣机控制面板', '冰箱温控标签', '电子产品铭牌', '酒类防伪标', '食品包装标签', '日化用品标签', '药品追溯标', '物流快递面单', '商超价签'];
    ...
    for (let i = 1; i <= TOTAL_ORDERS; i++) {
      ...
      const deliveryDate = new Date(new Date(orderDate).getTime() + randomInt(10, 45) * 86400000).toISOString().slice(0, 10);
      ...
      await conn.execute(
        ...
        [`ORD-${pad(i, 5)}`, orderDate, customer.id, `联系人${i}`, `1${randomInt(3000000000, 3999999999)}`, `收货地址${i}`, adminUser.id, amount, tax, totalWithTax, 0, 'CNY', randomItem(['月结30天', '月结60天', '货到付款']), deliveryDate, `CT-${now.getFullYear()}-${pad(i, 4)}`, status]
      );
      ...
      await conn.execute(
        ...
        [orderId, prodMat?.id || null, qty, prodMat?.unit || '张', unitPrice, taxRate, amount, tax, totalWithTax, status >= 3 ? Math.round(qty * 0.6) : 0, deliveryDate]
      );

      orderData.push({ id: orderId, orderNo: `ORD-${pad(i, 5)}`, customerId: customer.id, customerName: customer.customer_name, productName, qty, orderDate, deliveryDate, status });
    }
    ...
    for (let i = 1; i <= TOTAL_ORDERS; i++) {
      ...
      const planStart = new Date(new Date(od.orderDate).getTime() + randomInt(3, 10) * 86400000).toISOString().slice(0, 10);
      const planEnd = new Date(new Date(planStart).getTime() + randomInt(5, 20) * 86400000).toISOString().slice(0, 10);
      ...
      const actualEnd = status === 'completed' ? new Date(new Date(actualStart || planStart).getTime() + randomInt(3, 15) * 86400000).toISOString().slice(0, 10) : null;
      ...
      await conn.execute(
        ...
        [`WO-${pad(i, 5)}`, od.id, od.orderNo, od.customerName, od.productName, od.qty, '张', status, randomItem(['urgent', 'high', 'normal', 'low']), planStart, planEnd, actualStart, actualEnd, adminUser.id]
      );
      const [woRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
      woIds.push(woRow[0].id);
      woData.push({ id: woRow[0].id, woNo: `WO-${pad(i, 5)}`, orderId: od.id, orderNo: od.orderNo, customerName: od.customerName, productName: od.productName, qty: od.qty, status, planStart, planEnd });
    }
    ...
    for (let i = 1; i <= TOTAL_ORDERS; i++) {
      ...
      const inboundDate = new Date(new Date(wo.planStart).getTime() - randomInt(5, 15) * 86400000).toISOString().slice(0, 10);
      ...
      await conn.execute(
        ...
        [`INB-${pad(i, 5)}`, 'purchase', rawWh.id, supplier.id, supplier.supplier_name, null, null, 'po', totalAmount, totalQty, 'completed', inboundDate]
      );
      ...
      await conn.execute(
        ...
        [inbId, mainMat?.id, mainMat?.material_name, mainMat?.specification, batchNo, totalQty, mainMat?.unit || '张', mainMat?.purchase_price || 2, totalAmount]
      );
      ...
      await conn.execute(
        ...
        [labelNo, `INB-${pad(i, 5)}`, supplier.supplier_name, inboundDate, mainMat?.material_code, mainMat?.material_name, mainMat?.specification, mainMat?.unit || '张', batchNo, totalQty, width, lengthPerRoll, rawWh.id, 1, 0, 0, 1, width, 1]
      );
      const [lblRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
      labelIds.push(lblRow[0].id);
      labelData.push({ id: lblRow[0].id, labelNo, materialCode: mainMat?.material_code, materialName: mainMat?.material_name, batchNo, qty: totalQty, width, warehouseId: rawWh.id, supplierName: supplier.supplier_name, receiveDate: inboundDate });
      ...
      if (i <= 30) {
        ...
        await conn.execute(
          ...
          [inkLabelNo, `INB-${pad(i, 5)}`, supplier.supplier_name, inboundDate, inkMat?.material_code, inkMat?.material_name, inkMat?.specification, inkMat?.unit || 'kg', `INK-B${now.getFullYear()}${pad(i, 4)}`, inkQty, rawWh.id, 0, 0, 0, 2, 1]
        );
        const [inkLblRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
        labelIds.push(inkLblRow[0].id);
        labelData.push({ id: inkLblRow[0].id, labelNo: inkLabelNo, materialCode: inkMat?.material_code, materialName: inkMat?.material_name, batchNo: `INK-B${now.getFullYear()}${pad(i, 4)}`, qty: inkQty, width: null, warehouseId: rawWh.id, supplierName: supplier.supplier_name, receiveDate: inboundDate });
      }
    }
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      for (let j = 0; j < numCuts && remainW > 100; j++) {
        ...
        if (cw > 50) { cutWidths.push(cw); remainW -= cw; }
      }
      ...
      await conn.execute(
        ...
        [`CUT-${pad(i, 5)}`, srcLabel.id, srcLabel.labelNo, cutWidthStr, srcLabel.width, srcLabel.width - remainW, remainW, whUser.id, whUser.real_name, randomDate(yearStart, now), 1]
      );
      ...
      for (let j = 0; j < cutWidths.length; j++) {
        ...
        await conn.execute(
          ...
          [newLabelNo, srcLabel.labelNo.replace('LBL-', 'INB-'), srcLabel.supplierName, srcLabel.receiveDate, srcLabel.materialCode, srcLabel.materialName, null, 'M', srcLabel.batchNo, Math.round(srcLabel.qty * cutWidths[j] / srcLabel.width), cutWidths[j], srcLabel.warehouseId, 1, 0, 1, srcLabel.id, 1, cutWidths[j], 1]
        );
        const [newLblRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
        cuttingLabelIds.push(newLblRow[0].id);
        cuttingLabelData.push({ id: newLblRow[0].id, labelNo: newLabelNo, materialCode: srcLabel.materialCode, materialName: srcLabel.materialName, batchNo: srcLabel.batchNo, qty: Math.round(srcLabel.qty * cutWidths[j] / srcLabel.width), width: cutWidths[j], warehouseId: srcLabel.warehouseId, supplierName: srcLabel.supplierName, receiveDate: srcLabel.receiveDate });
    ...
    for (let i = 1; i <= TOTAL_ORDERS; i++) {
      ...
      await conn.execute(
        ...
        [cardNo, wo.id, wo.woNo, wo.productName.substring(0, 2) + '-' + pad(i, 3), wo.productName, mainMats[(i - 1) % mainMats.length]?.specification, wo.planStart, wo.qty, mainLabel.id, mainLabel.labelNo, burdeningStatus, lockStatus, prodUser.id, prodUser.real_name]
      );
      const [cardRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
      const cardId = cardRow[0].id;
      cardIds.push(cardId);
      cardData.push({ id: cardId, cardNo, woId: wo.id, woNo: wo.woNo, productName: wo.productName, planQty: wo.qty, mainLabelId: mainLabel.id, mainLabelNo: mainLabel.labelNo, burdeningStatus, lockStatus });
      ...
      await conn.execute(
        ...
        [cardId, cardNo, mainLabel.id, mainLabel.labelNo, 1, mainLabel.materialCode, mainLabel.materialName, mainMats[(i - 1) % mainMats.length]?.specification, mainLabel.batchNo, mainLabel.qty, 'M']
      );

      if (i <= 30) {
        const inkLabel = labelData.find((l: any) => l.materialCode?.startsWith('MAT-INK') && l.id > mainLabel.id);
        if (inkLabel) {
          await conn.execute(
            `INSERT INTO prd_process_card_material (card_id, card_no, label_id, label_no, material_type, material_code, material_name, specification, batch_no, quantity, unit) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [cardId, cardNo, inkLabel.id, inkLabel.labelNo, 2, inkLabel.materialCode, inkLabel.materialName, inkMats[(i - 1) % inkMats.length]?.specification, inkLabel.batchNo, inkLabel.qty, 'kg']
          );
        }
      }
    }
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      await conn.execute(
        ...
        ['burdening', label.labelNo, label.labelNo, card.cardNo, 1, '扫码配料成功', prodUser.id, prodUser.real_name, randomDate(yearStart, now)]
      );
    }
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      await conn.execute(
        ...
        [`MI-${pad(i, 5)}`, wo.id, wo.woNo, rawWh.id, issueDate, 1, 2, whUser.id, whUser.real_name, whUser.id]
      );
      ...
      await conn.execute(
        ...
        [miId, mainMats[(i - 1) % mainMats.length]?.id, mainLabel.materialCode, mainLabel.materialName, mainLabel.qty, issueQty, 'M', mainLabel.batchNo]
      );
      ...
      if (i <= 20) {
        ...
        if (inkLabel) {
          await conn.execute(
            `INSERT INTO prd_material_issue_item (issue_id, material_id, material_code, material_name, required_qty, issued_qty, unit, batch_no) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [miId, inkMats[(i - 1) % inkMats.length]?.id, inkLabel.materialCode, inkLabel.materialName, inkLabel.qty, Math.round(inkLabel.qty * 0.5), 'kg', inkLabel.batchNo]
          );
        }
      }
    }
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      await conn.execute(
        ...
        [`WR-${pad(i, 5)}`, wo.id, wo.woNo, randomItem(['丝印', '模切', '分条', '覆膜', '品检']), randomInt(1, 5), null, prodUser.id, prodUser.real_name, planQty, completedQty, qualifiedQty, defectiveQty, scrapQty, `${reportDate} 08:00:00`, `${reportDate} 17:00:00`, randomAmount(7, 9), i % 5 === 0 ? 1 : 0, i % 5 === 0 ? 1 : null, i % 5 === 0 ? qcUser.real_name : null]
      );
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      await conn.execute(
        ...
        [`PI-${pad(i, 5)}`, wo.id, wo.woNo, finishedWh.id, inboundDate, 1, 2, whUser.id, whUser.real_name, whUser.id]
      );
      ...
      await conn.execute(
        ...
        [piId, prodMat?.id, prodMat?.material_code, prodMat?.material_name, prodQty, '张', prodBatchNo, prodQty]
      );
      ...
      await conn.execute(
        ...
        [prodLabelNo, wo.id, wo.woNo, prodMat?.id, prodMat?.material_code, prodMat?.material_name, prodQty, '张', prodBatchNo, 1, 1, 1, whUser.id]
      );
    }
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      await conn.execute(
        ...
        [`TR-${pad(i, 5)}`, card.id, card.cardNo, wo.woNo, wo.productName.substring(0, 2) + '-' + pad(i, 3), mainLabel.id, 1, qcUser.id, qcUser.real_name, `${traceDate} 10:00:00`]
      );
      ...
      await conn.execute(
        ...
        [trId, mainLabel.id, mainLabel.labelNo, mainLabel.materialCode, mainLabel.materialName, mainMats[(i - 1) % mainMats.length]?.specification, mainLabel.batchNo, mainLabel.supplierName, mainLabel.receiveDate, 1]
      );
      ...
      if (inkLabel) {
        await conn.execute(
          `INSERT INTO inv_trace_detail (trace_id, label_id, label_no, material_code, material_name, specification, batch_no, supplier_name, receive_date, material_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [trId, inkLabel.id, inkLabel.labelNo, inkLabel.materialCode, inkLabel.materialName, inkMats[(i - 1) % inkMats.length]?.specification, inkLabel.batchNo, inkLabel.supplierName, inkLabel.receiveDate, 2]
        );
      }
    }
    ...
    for (let i = 1; i <= Math.min(materials.length, 50); i++) {
      ...
      await conn.execute(
        ...
        [mat.id, mat.material_name, wh.id, wh.warehouse_code, qty, qty, 0, mat.unit, mat.purchase_price, Math.round(qty * mat.purchase_price * 100) / 100, 100]
      );
    }
