...
export const POST = withPermission(async (_request: NextRequest) => {
  ...
  const result = await transaction(async (conn) => {
    ...
    const matCats: DbRow[] = matCatRows;
    ...
    const whUser = userRows.find((u: DbRow) => u.username === 'zhaolei') || userRows[1];
    const prodUser = userRows.find((u: DbRow) => u.username === 'wangqiang') || userRows[2];
    const qcUser = userRows.find((u: DbRow) => u.username === 'zhoujie') || userRows[3];
    ...
    for (let i = 1; i <= 50; i++) {
      ...
      const catType = matCats.find((c: DbRow) => c.category_type === mt.cat_type);
    ...
    const mainMats = materials.filter((m: DbRow) => m.cat_type === 1);
    const inkMats = materials.filter((m: DbRow) => m.cat_type === 2);
    const _auxMats = materials.filter((m: DbRow) => m.cat_type === 3);
    const productMats = materials.filter((m: DbRow) => m.cat_type === 4);
    ...
    const warehouses: DbRow[] = Object.entries(whMap).map(([code, id]) => ({
      id,
      warehouse_code: code,
    }));
    ...
    for (let i = 1; i <= TOTAL_ORDERS; i++) {
      ...
      const [inbRow] = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      const [cutRow] = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      const [miRow] = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      const [piRow] = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      const [trRow] = await conn.execute('SELECT LAST_INSERT_ID() as id');
