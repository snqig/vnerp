...
export const POST = withErrorHandler(async (request: NextRequest) => {
  const result = await transaction(async (conn) => {
    ...
    const [custRows]: any = await conn.execute('SELECT id, customer_code, customer_name FROM crm_customer ORDER BY id');
    const customers: any[] = custRows;

    const [suppRows]: any = await conn.execute('SELECT id, supplier_code, supplier_name FROM pur_supplier ORDER BY id');
    ...
    const [whRows]: any = await conn.execute('SELECT id, warehouse_code, warehouse_name FROM inv_warehouse ORDER BY id');
    const warehouses: any[] = whRows;

    const [woRows]: any = await conn.execute('SELECT id, work_order_no FROM prod_work_order ORDER BY id');
    const workOrders: any[] = woRows;

    const [soRows]: any = await conn.execute('SELECT id, order_no, customer_id FROM sal_order ORDER BY id');
    const salesOrders: any[] = soRows;

    const [eqpRows]: any = await conn.execute('SELECT id, equipment_code, equipment_name FROM eqp_equipment ORDER BY id');
    let equipment: any[] = eqpRows;
    if (equipment.length === 0) {
      const eqpTypes = [
        { code: 'SMP', name: '半自动丝印机' }, { code: 'SMP', name: '全自动丝印机' },
        { code: 'DIE', name: '平压平模切机' }, { code: 'DRY', name: 'UV固化机' },
        { code: 'INS', name: '品检机' }, { code: 'AUX', name: '覆膜机' },
      ];
      for (let i = 1; i <= 20; i++) {
        ...
        await conn.execute(
          ...
          [`EQP-${et.code}-${String(i).padStart(4, '0')}`, et.name, (i % 5) + 1, randomItem(['东远', '永创', '精工']), `MODEL-${String(i).padStart(3, '0')}`, `SN-${String(i).padStart(6, '0')}`, null, randomItem(['丝印车间', '模切车间', '品检车间']), randomDate(new Date(2019, 0, 1), new Date()), `${randomItem(['东远', '永创'])}制造`, randomDate(new Date(), new Date(2028, 11, 31)), randomInt(500, 5000), 1, randomAmount(60, 95), randomAmount(85, 99), randomAmount(75, 98), randomAmount(90, 99.9), randomInt(1000, 50000), randomDate(new Date(2025, 0, 1), new Date()), randomDate(new Date(), new Date(2027, 11, 31)), 1]
        );
      }
      const [newEqpRows]: any = await conn.execute('SELECT id, equipment_code, equipment_name FROM eqp_equipment ORDER BY id');
      equipment = newEqpRows;
    }

    const [userRows]: any = await conn.execute('SELECT id, username, real_name FROM sys_user ORDER BY id');
    ...
    const productNames = [
      '空调控制面板标签', '洗衣机控制面板', '手机电池标签', '新能源电池标签', '医疗设备面板',
      '工业设备铭牌', '电子元器件标签', '电池防伪标签', '汽车仪表盘面板', '智能家居面板',
      '冰箱温控面板', '微波炉控制面板', '电视背光模组标签', '笔记本电脑铭牌', '无人机操控面板',
      '充电桩标识标签', '安防设备面板', '电动工具铭牌', '智能手表表盘', '蓝牙耳机标签',
    ];
    const productSpecs = [
      '120×80mm', '180×100mm', '50×30mm', '150×80mm', '200×150mm',
      '100×60mm', '40×20mm', '80×50mm', '250×120mm', '150×100mm',
      '160×90mm', '200×80mm', '300×200mm', '80×40mm', '120×60mm',
      '200×150mm', '100×80mm', '60×40mm', '45×45mm', '30×15mm',
    ];
    const productUnits = ['张', '张', '张', '张', '张', '张', '张', '张', '张', '张', '张', '张', '张', '张', '张', '张', '张', '张', '片', '张'];
    const categoryNames = ['丝印标签', '丝印面板', '丝印铭牌', '导电线路', '防伪标签'];
    const shortNames = ['空调面板', '洗衣机面板', '手机电池标', '电池标签', '医疗面板', '工业铭牌', '元器件标', '防伪标', '仪表盘', '智能面板', '冰箱面板', '微波炉面板', '电视标签', '笔记本铭牌', '无人机面板', '充电桩标', '安防面板', '工具铭牌', '手表表盘', '耳机标'];
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `PRD-${pad(i + 1, 5)}`, productNames[i], shortNames[i], productSpecs[i], productUnits[i],
          (i % 5) + 1, categoryNames[i % 5], cust.id, cust.customer_name,
          `V${(i % 3) + 1}.0`, `${productNames[i]}，丝印产品`, randomItem(['active', 'active', 'active', 'inactive']),
          randomAmount(0.5, 15), randomAmount(1, 30), randomInt(500, 5000), defaultUserId
        ]
      );
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const orderMonth = parseInt(orderDate.substring(0, 4)) * 100 + parseInt(orderDate.substring(5, 7));
      const requiredDate = new Date(new Date(orderDate).getTime() + randomInt(7, 30) * 86400000).toISOString().slice(0, 10);
      ...
      await conn.execute(
        ...
        [
          `SMP-2026-${pad(i + 1, 5)}`, orderMonth, orderDate, randomItem(sampleTypes), cust.customer_name,
          randomItem(printMethods), randomItem(colorSequences), productNames[i] + '(打样)',
          mat.material_code, productSpecs[i], mat.material_name,
          `SO-SMP-${pad(i + 1, 5)}`, requiredDate, randomItem(progressStatuses),
          randomItem([0, 1]), randomItem([0, 1, 0]), randomItem([0, 1, 0]),
          randomInt(10, 100), `制版→印刷→质检→寄样`, randomInt(1, 5),
          randomItem(sampleReasons), randomItem(trackers), randomItem(providedMaterials),
          `${String(randomInt(8, 17)).padStart(2, '0')}:${String(randomInt(0, 59)).padStart(2, '0')}:00`,
          randomItem(mylarInfos), randomItem(sampleStocks), randomItem(customerConfirms),
          `${productNames[i]}打样需求，需首件确认`, randomInt(0, 3)
        ]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const nextDate = new Date(now.getTime() + randomInt(7, 30) * 86400000).toISOString().slice(0, 10);
      await conn.execute(
        ...
        [cust.id, cust.customer_name, randomItem(followTypes), `跟进客户${cust.customer_name}，沟通丝印标签订单需求及交期安排`, `联系人${i + 1}`, user.real_name, nextDate, randomItem(['新品开发机会', '批量订单机会', '长期合作意向', '样品确认中']), randomItem([1, 2]), `客户反馈良好，需持续跟进`, defaultUserId]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          cust.id, cust.customer_name, period, pStart, pEnd,
          randomInt(5, 50), randomAmount(50000, 2000000),
          randomInt(4, 45), randomInt(0, 5), randomInt(0, 3),
          randomAmount(85, 99.5), randomAmount(3.0, 5.0),
          randomItem(customerLevels), randomAmount(-10, 30)
        ]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `DT-${pad(i + 1, 5)}`, `${productNames[i]}丝网版`, (i % 2) + 1, productSpecs[i],
          randomItem(templateMaterials), maxUsage, remainingUsage, warningUsage,
          1, randomItem(storageLocations), randomDate(yearStart, now), sup.id, defaultUserId
        ]
      );
      const [idRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const nextMaintenanceDate = new Date(new Date(lastMaintenanceDate).getTime() + maintenanceDays * 86400000).toISOString().slice(0, 10);
      await conn.execute(
        ...
        [
          `DIE-${pad(i + 1, 5)}`, `${productNames[i]}刀模`, (i % 2) + 1, productSpecs[i],
          cust.id, productNames[i], maxUseCount, usedCount, remainingCount,
          maintenanceDays, lastMaintenanceDate, nextMaintenanceDate, wh.id, 1
        ]
      );
    }
    stats.prd_die = 20;
    ...
    const formulaNames = [
      '丝印黑色标准配方', '丝印白色UV配方', '导电银浆配方', '丝印红色溶剂配方', '丝印蓝色UV配方',
      '丝印金色溶剂配方', 'UV透明光油配方', '丝印绿色UV配方', '丝印黄色溶剂配方', '导电碳浆配方',
      '丝印银色UV配方', '丝印紫色溶剂配方', '防伪荧光配方', '磁性油墨配方', '温变油墨配方',
      '光变油墨配方', '夜光油墨配方', '导热油墨配方', '绝缘油墨配方', '可剥胶配方',
    ];
    const pantoneCodes = ['PANTONE Black C', 'PANTONE White C', 'PANTONE 877 C', 'PANTONE 185 C', 'PANTONE 286 C', 'PANTONE 871 C', 'PANTONE 801 C', 'PANTONE 356 C', 'PANTONE 109 C', 'PANTONE 432 C', 'PANTONE 877 C', 'PANTONE 2685 C', 'PANTONE 802 C', 'PANTONE Black 4 C', 'PANTONE 11-0601 C', 'PANTONE 16-1546 C', 'PANTONE 3755 C', 'PANTONE Cool Gray 8 C', 'PANTONE 11-0601 C', 'PANTONE 801 C'];
    const colorNames = ['黑色', '白色', '银色', '红色', '蓝色', '金色', '透明', '绿色', '黄色', '碳黑', '银色', '紫色', '荧光绿', '磁性黑', '温变', '光变', '夜光绿', '导热灰', '绝缘白', '透明'];
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `IF-2026-${pad(i + 1, 5)}`, formulaNames[i], pantoneCodes[i], colorNames[i],
          `C${pad(i + 1, 3)}`, inkType, baseInkTypes[inkTypes.indexOf(inkType)],
          randomAmount(1, 50), 'kg', randomItem([48, 72, 168, 336, 720]), 1
        ]
      );
      const [idRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    const inkMaterials = materials.filter((m: any) => m.material_name?.includes('油墨') || m.material_name?.includes('银浆') || m.material_name?.includes('光油') || m.material_name?.includes('感光胶'));
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const expireTime = new Date(new Date(openTime).getTime() + expireHours * 3600000).toISOString().slice(0, 19).replace('T', ' ');
      const user = users[i % users.length];
      ...
      await conn.execute(
        ...
        [
          `IOR-2026-${pad(i + 1, 5)}`, mat.id, mat.material_code, mat.material_name,
          `B2026${pad(i + 1, 4)}`, randomItem(inkTypes), openTime, expireHours, expireTime,
          remainingQty, 'kg', randomItem([1, 2, 3]), user.id, user.real_name
        ]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const expireTime = new Date(new Date(mixTime).getTime() + randomItem([48, 72, 168]) * 3600000).toISOString().slice(0, 19).replace('T', ' ');
      await conn.execute(
        ...
        [
          `IMR-2026-${pad(i + 1, 5)}`, mat.id, mat.material_code, mat.material_name,
          randomItem(['1:2:1', '3:1', '2:1:1', '4:1', '1:1']),
          colorNames[i % colorNames.length], `C${pad(i + 1, 3)}`,
          cust.id, cust.customer_name, mixTime, user.id, user.real_name,
          quantity, 'kg', wh.id, randomItem([1, 2]), expireTime
        ]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          productIds[pIdx], `PRD-${pad(pIdx + 1, 5)}`, productNames[pIdx], stage, 1,
          `V${(i % 3) + 1}.0`, randomItem(['material', 'process', 'design', 'specification']),
          `${productNames[i % 20]}生命周期阶段变更`, `${productNames[i % 20]}进入${lifecycleStageNames[i % 5]}`,
          defaultUserName, `${randomDate(yearStart, now)} ${String(randomInt(8, 17)).padStart(2, '0')}:00:00`,
          effectiveDate, `${productNames[pIdx]}处于${stage}阶段`, defaultUserId
        ]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const approveTime = statusVal >= 2 ? `${randomDate(yearStart, now)} ${String(randomInt(8, 17)).padStart(2, '0')}:${String(randomInt(0, 59)).padStart(2, '0')}:00` : null;
      await conn.execute(
        ...
        [
          `ECO-2026-${pad(i + 1, 5)}`, ecoType, productIds[pIdx], `PRD-${pad(pIdx + 1, 5)}`, productNames[pIdx],
          oldVer, newVer, `${productNames[pIdx]}${ecoType}变更需求`, `变更${ecoType}相关参数以满足客户要求`,
          `对生产计划及物料采购有轻微影响`, statusVal, user.real_name, applyTime,
          statusVal >= 2 ? defaultUserName : null, approveTime, null, defaultUserId
        ]
      );
    }
    stats.plm_eco = 20;
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `BOM-2026-${pad(i + 1, 5)}`, productIds[pIdx], `PRD-${pad(pIdx + 1, 5)}`, productNames[pIdx],
          productSpecs[pIdx], `V${(i % 3) + 1}.0`, i < 10 ? 1 : 0, randomItem([10, 20, 20]),
          productUnits[pIdx], 1, totalMaterialCount, totalCost, `${productNames[pIdx]}BOM`, defaultUserId
        ]
      );
      const [idRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    for (let i = 0; i < materials.length; i++) {
      ...
      await conn.execute(
        ...
        [mat.material_code, mat.material_name, mat.specification, matType, randomItem(['薄膜', '油墨', '溶剂', '辅料']), mat.unit, mat.purchase_price || 0, 100, 1]
      );
    ...
    const processNames = ['丝印', '模切', 'UV固化', '烫印', '检品', '分条', '覆膜', '冲压', '烘干', '包装'];
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          bomId, i + 1, null, 1, bmId, mat.material_code, mat.material_name,
          mat.specification, mat.unit, consumptionQty, lossRate, actualQty,
          unitCost, totalCost, randomItem(materialTypes), randomItem([0, 1]),
          `P${pad(i + 1, 2)}`, i + 1, randomItem(processNames)
        ]
      );
    }
    stats.bom_line = 20;
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const [custRow]: any = await conn.execute('SELECT customer_name FROM crm_customer WHERE id = ?', [so.customer_id]);
      ...
      await conn.execute(
        ...
        [
          `DN-2026-${pad(i + 1, 5)}`, so.id, so.order_no, so.customer_id, custName,
          randomDate(yearStart, now), `联系人${i + 1}`, `0769-${randomInt(22000000, 22999999)}`,
          `广东省东莞市长安镇${randomItem(['乌沙路', '沙头路', '锦厦路'])}${randomInt(1, 200)}号`,
          wh.id, randomItem(logisticsCompanies), `SF${randomInt(1000000000, 9999999999)}`,
          randomItem(['张师傅', '李师傅', '王师傅']), `粤S${randomInt(10000, 99999)}`,
          totalQty, totalAmount, signStatus,
          signStatus === 1 ? `签收人${i + 1}` : null,
          signStatus === 1 ? `${randomDate(yearStart, now)} ${String(randomInt(8, 17)).padStart(2, '0')}:00:00` : null,
          signStatus === 1 ? '正常签收' : null,
          randomItem([1, 2, 3]), defaultUserId
        ]
      );
      const [idRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [deliveryId, mat.id, mat.material_name, mat.specification, quantity, mat.unit, unitPrice, amount, `B2026${pad(i + 1, 4)}`]
      );
    }
    stats.sal_delivery_order_item = 20;

    const returnReasons = ['丝印偏色超出标准', '套位偏差不合格', '模切毛刺不符合要求', '尺寸偏差超标', '表面划痕影响品质', '油墨附着力不足', '色差超出客户标准', '印刷内容错误', '材料规格不符', '客户需求变更'];
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `RT-2026-${pad(i + 1, 5)}`, so.id, so.order_no, deliveryId, `DN-2026-${pad((i % deliveryIds.length) + 1, 5)}`,
          so.customer_id, custName, randomDate(yearStart, now), randomItem([1, 2, 3]),
          randomItem(returnReasons), totalQty, totalAmount,
          randomItem([0, 1]), randomItem([null, 1, 2]), wh.id,
          randomItem([0, 1]), randomItem([1, 2, 3]), defaultUserId
        ]
      );
      const [idRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [returnId, mat.id, mat.material_name, mat.specification, quantity, mat.unit, unitPrice, amount, `B2026${pad(i + 1, 4)}`]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const periodEnd = new Date(new Date(periodStart).getTime() + randomInt(28, 31) * 86400000).toISOString().slice(0, 10);
      ...
      await conn.execute(
        ...
        [
          `RC-2026-${pad(i + 1, 5)}`, cust.id, cust.customer_name, periodStart, periodEnd,
          deliveryAmount, returnAmount, discountAmount, netAmount, receivedAmount, balanceAmount,
          confirmStatus, confirmStatus === 1 ? defaultUserName : null,
          confirmStatus === 1 ? `${randomDate(yearStart, now)} ${String(randomInt(8, 17)).padStart(2, '0')}:00:00` : null,
          confirmStatus === 1 ? '对账确认无误' : null,
          randomItem([1, 2]), defaultUserId
        ]
      );
      const [idRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const sourceNo = sourceType === 1 ? `DN-2026-${pad((i % 20) + 1, 5)}` : `RT-2026-${pad((i % 20) + 1, 5)}`;
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `MR-2026-${pad(i + 1, 5)}`, wo.id, wo.work_order_no, wh.id,
          randomDate(yearStart, now), randomItem([1, 2, 3]),
          user.id, user.real_name, `生产退料-${wo.work_order_no}`, defaultUserId
        ]
      );
      const [idRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [returnId, mat.id, mat.material_code, mat.material_name, randomInt(1, 500), mat.unit, `B2026${pad(i + 1, 4)}`]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `EMR-2026-${pad(i + 1, 5)}`, null, eq.id, randomItem([1, 2, 3, 4, 5]),
          `${eq.equipment_name}定期保养检查`, `清洁润滑关键部件，检查磨损情况并校准参数`,
          startTime, endTime, downtimeHours, randomAmount(200, 5000),
          user.id, randomItem([1, 1, 2])
        ]
      );
    }
    stats.eqp_maintenance_record = 20;

    const faultDescs = ['丝印刮刀磨损异常', 'UV灯管亮度不足', '模切刀模断裂', '传送带跑偏', '温控系统失灵', '对位系统精度下降', '导轨润滑不足', '传感器灵敏度降低', '电机异响', '气缸漏气'];
    for (let i = 0; i < 20; i++) {
      const eq = equipment[i % equipment.length];
      ...
      await conn.execute(
        ...
        [
          `ERP-2026-${pad(i + 1, 5)}`, eq.id, eq.equipment_code, eq.equipment_name,
          randomDate(yearStart, now), randomItem(faultDescs), randomItem([1, 2]),
          randomItem(['张师傅', '李师傅', '王师傅', '赵工', '陈工']),
          repairStartTime, repairEndTime, repairCost, '更换损坏部件，重新校准设备参数',
          randomItem([1, 2]), defaultUserId
        ]
      );
    }
    ...
    const calibrationOrgs = ['广东省计量科学研究院', '深圳市计量质量检测研究院', '东莞市计量所', '国家印刷机械质量检验中心', '第三方校准机构'];
    for (let i = 0; i < 20; i++) {
      const eq = equipment[i % equipment.length];
      const cDate = randomDate(yearStart, now);
      const nextCDate = new Date(new Date(cDate).getTime() + randomInt(90, 365) * 86400000).toISOString().slice(0, 10);
      await conn.execute(
        ...
        [
          `ECR-2026-${pad(i + 1, 5)}`, eq.id, eq.equipment_code, eq.equipment_name,
          cDate, nextCDate, randomItem(calibrationOrgs), randomItem([1, 1, 1, 2]),
          `CERT-2026-${pad(i + 1, 5)}`, randomAmount(500, 3000), randomItem([1, 2]), defaultUserId
        ]
      );
    }
    stats.eqp_calibration = 20;

    const scrapReasons = ['设备老化无法维修', '技术淘汰无法满足生产要求', '多次维修仍无法正常使用', '安全风险无法继续使用', '精度严重下降无法校准'];
    for (let i = 0; i < 20; i++) {
      const eq = equipment[i % equipment.length];
      ...
      await conn.execute(
        ...
        [
          `ESR-2026-${pad(i + 1, 5)}`, eq.id, eq.equipment_code, eq.equipment_name,
          randomDate(yearStart, now), randomItem(scrapReasons), originalValue, netValue,
          defaultUserName, randomItem([1, 2]), defaultUserId
        ]
      );
    }
    stats.eqp_scrap = 20;
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `TRF-2026-${pad(i + 1, 5)}`, fromWh.id, toWh.id, randomDate(yearStart, now),
          randomItem([1, 2]), randomItem([1, 2, 3]), user.id, user.real_name,
          `调拨备注`, defaultUserId
        ]
      );
      const [idRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [transferId, mat.id, mat.material_code, mat.material_name, randomInt(10, 5000), mat.unit, `B2026${pad(i + 1, 4)}`]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `STK-2026-${pad(i + 1, 5)}`, wh.id, randomDate(yearStart, now),
          randomItem([1, 2, 3]), randomItem([1, 2, 3]), user.id, user.real_name,
          `盘点备注`, defaultUserId
        ]
      );
      const [idRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [stkId, mat.id, mat.material_code, mat.material_name, systemQty, actualQty, diff, mat.unit, `B2026${pad(i + 1, 4)}`]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `ADJ-2026-${pad(i + 1, 5)}`, wh.id, randomDate(yearStart, now),
          randomItem([1, 2, 3]), randomItem([1, 2]), user.id, user.real_name,
          `调整备注`, defaultUserId
        ]
      );
      const [idRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [adjustId, mat.id, mat.material_code, mat.material_name, beforeQty, adjustQty, afterQty, mat.unit, `B2026${pad(i + 1, 4)}`]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `SOB-2026-${pad(i + 1, 5)}`, so.id, so.order_no, so.customer_id, custName,
          wh.id, randomDate(yearStart, now), user.real_name, randomItem([1, 2, 3]), defaultUserId
        ]
      );
      const [idRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [outboundId, mat.id, mat.material_code, mat.material_name, randomInt(100, 50000), mat.unit, `B2026${pad(i + 1, 4)}`]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      const sup = suppliers[i % suppliers.length];
      const orderDate = randomDate(yearStart, now);
      const deliveryDate = new Date(new Date(orderDate).getTime() + randomInt(7, 30) * 86400000).toISOString().slice(0, 10);
      ...
      await conn.execute(
        ...
        [
          `PO-2026-${pad(i + 1, 5)}`, sup.id, sup.supplier_name, sup.supplier_code,
          orderDate, deliveryDate, 'CNY', totalAmount, totalQuantity,
          taxRate, taxAmount, grandTotal, randomItem([10, 20, 30, 40]),
          randomItem(paymentTermsList), `联系人${i + 1}`, `0769-${randomInt(22000000, 22999999)}`, defaultUserId
        ]
      );
      const [idRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [poId, i + 1, mat.id, mat.material_code, mat.material_name, mat.specification, mat.unit, orderQty, receivedQty, unitPrice, amount, taxRate, taxAmount, lineTotal, requireDate]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const expectedDate = new Date(new Date(requestDate).getTime() + randomInt(7, 30) * 86400000).toISOString().slice(0, 10);
      await conn.execute(
        ...
        [
          `PR-2026-${pad(i + 1, 5)}`, requestDate, randomItem(['material', 'equipment', 'supply']),
          randomItem(requestDepts), user.real_name, totalAmount, 'CNY',
          randomItem([0, 1, 2, 3, 4]), randomItem([1, 2, 3, 4]),
          expectedDate, sup.supplier_name, defaultUserId
        ]
      );
      const [idRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [reqId, i + 1, mat.material_code, mat.material_name, mat.specification, mat.unit, quantity, price, amount, sup.supplier_name, expectedDate]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `QFI-2026-${pad(i + 1, 5)}`, randomDate(yearStart, now), wo.id, wo.work_order_no,
          productIds[pIdx], `PRD-${pad(pIdx + 1, 5)}`, productNames[pIdx],
          `B2026${pad(i + 1, 4)}`, inspectionQty, qualifiedQty, unqualifiedQty,
          randomItem([1, 1, 2, 3]), user.real_name, defaultUserId
        ]
      );
      const [idRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    const defectTypes = ['丝印偏色', '套位偏差', '模切毛刺', '表面划痕', '油墨脱落', '尺寸偏差', '附着力不足', '色差超标', '印刷模糊', '漏印'];
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `UQ-2026-${pad(i + 1, 5)}`, inspId, 'final_inspection',
          `QFI-2026-${pad((i % inspectionIds.length) + 1, 5)}`, mat.id, mat.material_name,
          quantity, randomItem(defectTypes), `${mat.material_name}出现${randomItem(defectTypes)}缺陷`,
          randomItem([1, 2, 3]), randomItem([1, 2]), randomItem(['张质检', '李质检', '王质检', '赵主管']),
          handleDate
        ]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `UQH-2026-${pad(i + 1, 5)}`, inspId, mat.id, mat.material_code, mat.material_name,
          unqualifiedQty, randomItem([1, 2, 3]), randomItem([1, 2]),
          randomItem(requestDepts), user.real_name,
          `对${mat.material_name}不合格品进行处理`, costAmount, defaultUserId
        ]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const closeTime = status === 4 ? `${randomDate(yearStart, now)} ${String(randomInt(8, 17)).padStart(2, '0')}:${String(randomInt(0, 59)).padStart(2, '0')}:00` : null;
      await conn.execute(
        ...
        [
          `CP-2026-${pad(i + 1, 5)}`, cust.id, cust.customer_name, so.order_no,
          `PRD-${pad(pIdx + 1, 5)}`, productNames[pIdx], randomItem(complaintTypes), randomItem(complaintLevels),
          `${productNames[pIdx]}出现${randomItem(defectTypes)}问题`, defectQty, totalQty, defectRate,
          user.real_name, reportTime, randomItem(['品质经理赵六', '品质主管李四', '品质工程师王五']),
          '隔离不合格品并通知客户', randomItem(['制程控制不足', '原材料批次问题', '设备参数偏差', '操作人员失误']),
          '加强制程巡检频次', '建立预防性检查机制', randomItem(['有效', '需持续跟踪']),
          defaultUserName, reportTime, status, closeTime, defaultUserId
        ]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `LT-2026-${pad(i + 1, 5)}`, randomItem(testTypes), productIds[pIdx],
          `PRD-${pad(pIdx + 1, 5)}`, productNames[pIdx], `B2026${pad(i + 1, 4)}`,
          randomItem(sampleSources), randomItem(['附着力测试/百格测试', '耐磨测试/摩擦测试', '色差检测/光谱分析', '粘度测试/流变分析', '厚度测量/千分尺检测']),
          randomItem(['合格', '合格', '合格', '不合格']), randomItem(['qualified', 'qualified', 'unqualified']),
          user.real_name, testTime, reviewer.real_name, reviewTime,
          randomItem(['百格刀', '摩擦试验机', '色差仪', '粘度计', '千分尺']), defaultUserId
        ]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const deadline = new Date(new Date(randomDate(yearStart, now)).getTime() + randomInt(14, 60) * 86400000).toISOString().slice(0, 10);
      await conn.execute(
        ...
        [
          `SA-2026-${pad(i + 1, 5)}`, sup.id, sup.supplier_name, randomItem(auditTypes),
          randomItem(['质量管理体系', '生产过程控制', '原材料管控', '环境与安全']),
          randomDate(yearStart, now), user.real_name, '质量体系/过程控制/产品检验/交付能力',
          `体系${randomInt(15, 25)}/过程${randomInt(15, 25)}/检验${randomInt(15, 25)}/交付${randomInt(15, 25)}`,
          totalScore, randomItem(['qualified', 'conditional', 'unqualified']),
          randomItem(['无', '文件管控不足', '检验记录不完整', '交付偶尔延迟']),
          randomItem(['完善文件管控流程', '补充检验记录', '优化交付计划']),
          deadline, randomItem([1, 2]), defaultUserId
        ]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `QR-${pad(i + 1, 6)}`, randomItem(qrTypes), productIds[pIdx], `PRD-${pad(pIdx + 1, 5)}`,
          `B2026${pad(i + 1, 4)}`, mat.id, mat.material_code, mat.material_name, mat.specification,
          randomInt(100, 50000), mat.unit, wh.id, wh.warehouse_name,
          sup.id, sup.supplier_name, cust.id, cust.customer_name,
          wo.id, wo.work_order_no, randomDate(yearStart, now), 1, defaultUserId
        ]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [`AR-2026-${pad(i + 1, 5)}`, 1, so.order_no, cust.id, amount, receivedAmount, balance, dueDate, status]
      );
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      await conn.execute(
        ...
        [
          `CR-2026-${pad(i + 1, 5)}`, costType, sourceType, wo.work_order_no, wo.id,
          randomItem(departments), amount, randomDate(yearStart, now),
          `${costType}成本-${wo.work_order_no}`, 1, defaultUserId
        ]
      );
    }
    stats.fin_cost_record = 20;

    const trainingNames = ['新员工入职培训', '安全生产培训', 'ERP系统操作培训', '品质标准培训', '丝印工艺培训', '5S管理培训', '消防安全培训', '设备操作培训', '化学品安全培训', '团队建设培训', '管理技能培训', '成本控制培训', '客户服务培训', '精益生产培训', 'ISO质量体系培训', '丝印调墨培训', '模切操作培训', '仓库管理培训', '采购流程培训', '财务制度培训'];
    const trainers = ['张伟', '刘洋', '王强', '周杰', '赵磊', '吴芳', '外部讲师'];
    const trainingPlaces = ['公司培训室A', '公司培训室B', '生产车间', '会议室1', '会议室2', '线上培训'];
    ...
    for (let i = 0; i < 20; i++) {
      await conn.execute(
        ...
        [
          `TRN-2026-${pad(i + 1, 5)}`, trainingNames[i], randomItem([1, 2, 3, 4]),
          randomDate(yearStart, now), randomItem([2, 4, 6, 8, 16, 24]),
          randomItem(trainers), `${trainingNames[i]}内容，涵盖理论讲解和实操演练`,
          randomItem(trainingPlaces), 1, defaultUserId
        ]
      );
      const [idRow]: any = await conn.execute('SELECT LAST_INSERT_ID() as id');
