...
export const POST = withPermission(async (_request: NextRequest, _userInfo) => {
  const result = await transaction(async (conn) => {
    ...
    for (const tr of transList) {
      await conn.execute(
        `INSERT INTO inv_inventory_transaction (trans_no, trans_type, source_type, material_id, material_code, warehouse_id, quantity, unit_cost, total_cost, reference_no, create_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
        [
          tr.no,
          tr.type,
          tr.stype,
