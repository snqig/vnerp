...
export const POST = withPermission(async (_request: NextRequest) => {
  const result = await transaction(async (conn) => {
    ...
    const [existingWarehouses] = await conn.execute(
    ...
    const [existingMaterials] = await conn.execute(
    ...
    const [_existingUsers] = await conn.execute('SELECT id, username FROM sys_user LIMIT 10');
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            ...
            colors[i - 1],
            inkTypes[(i - 1) % inkTypes.length],
            s.id,
            s.supplier_name || `供应商${i}`,
            `SK-${1000 + i}`,
            'kg',
            80 + i * 10,
            50 + i * 5,
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            ...
            (i % 3) + 1,
            altMat.id,
            altMat.material_code,
            altMat.material_name,
            1.0,
          ]
        );
      }
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            ...
            `第${i}次跟进，讨论新项目合作方案`,
            `联系人${i}`,
            `销售员${String.fromCharCode(65 + (i % 5))}`,
            '2026-05-01',
            `新项目机会-${i}`,
          ]
        );
    ...
    if (await isEmpty('eqp_calibration')) {
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            `CAL202604${String(i).padStart(3, '0')}`,
            e.id,
    ...
    if (await isEmpty('eqp_repair')) {
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            `RP202604${String(i).padStart(3, '0')}`,
            e.id,
            e.equipment_code || `EQP00${i}`,
            e.equipment_name || `设备${i}`,
            '2026-04-01',
    ...
    if (await isEmpty('eqp_scrap')) {
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            `SC202604${String(i).padStart(3, '0')}`,
            e.id,
            e.equipment_code || `EQP00${i}`,
            e.equipment_name || `设备${i}`,
            '2026-04-01',
    ...
    if (await isEmpty('hr_training')) {
      for (let i = 1; i <= 10; i++) {
        await conn.execute(
          ...
          [
            `TR202604${String(i).padStart(3, '0')}`,
            trainingNames[i - 1],
            (i % 3) + 1,
            '2026-04-01',
            4 + (i % 4),
            `讲师${i}`,
            `${trainingNames[i - 1]}课程内容`,
            `培训室${(i % 3) + 1}`,
          ]
        );
      }
    ...
    if (await isEmpty('hr_training_participant')) {
      const [trainingIds] = await conn.execute('SELECT id FROM hr_training LIMIT 10');
      const [employeeIds] = await conn.execute('SELECT id, name FROM sys_employee LIMIT 10');
      let tpCount = 0;
    ...
    if (await isEmpty('ink_mixed_record')) {
      const [baseInks] = await conn.execute('SELECT id, ink_code, ink_name FROM base_ink LIMIT 10');
      for (let i = 1; i <= 10; i++) {
        const bi = baseInks[(i - 1) % baseInks.length] || {
          id: i,
          ink_code: `INK00${i}`,
          ink_name: `油墨${i}`,
        };
        ...
        await conn.execute(
          ...
          [
            `MX202604${String(i).padStart(3, '0')}`,
            bi.id,
            bi.ink_code,
            bi.ink_name,
            `配方${i}:A${30 + i}%+B${40 - i}%+C${30}%`,
            colors[i - 1],
            `C${String(i).padStart(3, '0')}`,
            1,
            'DC印刷',
    ...
    if (await isEmpty('inv_production_inbound')) {
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            `PI202604${String(i).padStart(3, '0')}`,
            woItem.id,
            woItem.order_no || `WO00${i}`,
            w.id,
            '2026-04-01',
            `操作员${i}`,
          ]
        );
        const [piRows] = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    if (await isEmpty('inv_sales_outbound')) {
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            `SO202604${String(i).padStart(3, '0')}`,
            i,
        ...
        const [soRows] = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    if (await isEmpty('inv_stock_adjust')) {
      for (let i = 1; i <= 10; i++) {
        ...
        const [adjRows] = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    if (await isEmpty('inv_stocktaking')) {
      for (let i = 1; i <= 10; i++) {
        ...
        const [tkRows] = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    if (await isEmpty('inv_transfer_order')) {
      for (let i = 1; i <= 10; i++) {
        ...
        const [trRows] = await conn.execute('SELECT LAST_INSERT_ID() as id');
    ...
    if (await isEmpty('inv_trace_detail')) {
      const [traceRecords] = await conn.execute(
    ...
    if (await isEmpty('plm_eco')) {
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            `ECO202604${String(i).padStart(3, '0')}`,
            ecoTypes[(i - 1) % 5],
            p.id,
            p.product_code || `PRD00${i}`,
            p.product_name || `产品${i}`,
    ...
    if (await isEmpty('prd_material_issue')) {
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            `MI202604${String(i).padStart(3, '0')}`,
            woItem.id,
            woItem.order_no || `WO00${i}`,
            w.id,
            '2026-04-01',
        ...
        const [miRows] = await conn.execute('SELECT LAST_INSERT_ID() as id');
        const m = existingMaterials[(i - 1) % existingMaterials.length] || mat;
        await conn.execute(
    ...
    if (await isEmpty('prd_material_return')) {
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            `MR202604${String(i).padStart(3, '0')}`,
            woItem.id,
            woItem.order_no || `WO00${i}`,
            w.id,
            '2026-04-01',
        ...
        const [mrRows] = await conn.execute('SELECT LAST_INSERT_ID() as id');
        const m = existingMaterials[(i - 1) % existingMaterials.length] || mat;
        await conn.execute(
    ...
    if (await isEmpty('prd_product_label')) {
      const [existingLabelCount] = await conn.execute(
      ...
      if (existingLabelCount[0].cnt === 0) {
        for (let i = 1; i <= 10; i++) {
          ...
          await conn.execute(
            ...
            [
              `LBL202604${String(i).padStart(3, '0')}`,
              woItem.id,
              woItem.order_no || `WO00${i}`,
              m.id,
        ...
        if (processCards.length > 0 && productLabels.length > 0) {
          for (let i = 0; i < Math.min(10, processCards.length); i++) {
            ...
            await conn.execute(
              ...
              [
                processCards[i].id,
                processCards[i].card_no || `PC00${i + 1}`,
                pl.id,
                pl.label_no || `LBL00${i + 1}`,
                1,
                m.material_code || `MAT00${i + 1}`,
                m.material_name || `物料${i + 1}`,
    ...
    if (await isEmpty('pur_order')) {
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            `PO202604${String(i).padStart(3, '0')}`,
            '2026-04-01',
            s.id,
            `联系人${i}`,
    ...
    if (await isEmpty('pur_request')) {
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            `PR202604${String(i).padStart(3, '0')}`,
            '2026-04-01',
            `${(i % 3) + 1}`,
            `部门${i}`,
            `申请人${i}`,
    ...
    if (await isEmpty('pur_return_order')) {
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            `RTV202604${String(i).padStart(3, '0')}`,
            i,
            `PO202604${String(i).padStart(3, '0')}`,
            s.id,
            s.supplier_name || `供应商${i}`,
    ...
    if (await isEmpty('qc_incoming_inspection')) {
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            `QCI202604${String(i).padStart(3, '0')}`,
            '2026-04-01',
            s.id,
            s.supplier_name || `供应商${i}`,
            m.id,
    ...
    if (await isEmpty('qc_process_inspection')) {
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            `QCP202604${String(i).padStart(3, '0')}`,
            '2026-04-01',
            woItem.id,
            woItem.order_no || `WO00${i}`,
            processNames[i - 1],
    ...
    if (await isEmpty('qc_final_inspection')) {
      for (let i = 1; i <= 10; i++) {
        ...
        await conn.execute(
          ...
          [
            `QCF202604${String(i).padStart(3, '0')}`,
            '2026-04-01',
            woItem.id,
            woItem.order_no || `WO00${i}`,
            m.id,
    ...
    if (await isEmpty('qc_unqualified')) {
      const [unqualifiedRecords] = await conn.execute(
