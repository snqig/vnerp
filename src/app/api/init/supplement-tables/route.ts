...
export const POST = withPermission(async (_request: NextRequest) => {
  const result = await transaction(async (conn) => {
    ...
    await addColumn('inv_transfer_item', 'batch_id', "BIGINT UNSIGNED COMMENT '批次ID'");
    await addColumn('inv_transfer_item', 'location_id', "BIGINT UNSIGNED COMMENT '库位ID'");
    await addColumn(
      'inv_transfer_item',
      'original_inbound_date',
      "DATE COMMENT '原始入库日期(FIFO基准)'"
    );
    await addColumn('inv_transfer_item', 'qr_code', "VARCHAR(64) COMMENT '二维码'");
    await addIndex('inv_transfer_item', 'idx_transfer_batch', 'batch_id');

    // inv_stocktaking_item 补齐批次ID、原始入库日期、二维码字段
    await addColumn('inv_stocktaking_item', 'batch_id', "BIGINT UNSIGNED COMMENT '批次ID'");
    await addColumn('inv_stocktaking_item', 'original_inbound_date', "DATE COMMENT '原始入库日期'");
    await addColumn('inv_stocktaking_item', 'qr_code', "VARCHAR(100) COMMENT '二维码'");
    await addIndex('inv_stocktaking_item', 'idx_stk_batch', 'batch_id');

    // inv_stock_adjust_item 补齐批次、库位、原始入库日期、二维码字段
    await addColumn('inv_stock_adjust_item', 'batch_id', "BIGINT UNSIGNED COMMENT '批次ID'");
    await addColumn('inv_stock_adjust_item', 'location_id', "BIGINT UNSIGNED COMMENT '库位ID'");
    await addColumn(
      'inv_stock_adjust_item',
      'original_inbound_date',
      "DATE COMMENT '原始入库日期'"
    );
    await addColumn('inv_stock_adjust_item', 'qr_code', "VARCHAR(100) COMMENT '二维码'");
