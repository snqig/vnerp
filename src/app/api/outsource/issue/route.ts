...
export const GET = withPermission(async (request: NextRequest, _userInfo) => {
  ...
  if (outsourceOrderNo) {
    where += ' AND i.outsource_order_no LIKE ?';
  ...
  const totalRows = await query('SELECT COUNT(*) as total FROM outsource_issue i ' + where, params);
  ...
  for (const row of rows) {
    const items = await query('SELECT * FROM outsource_issue_item WHERE issue_id = ?', [row.id]);
    row.items = items;
  }
...
export const PUT = withPermission(
  async (request: NextRequest, _userInfo) => {
      ...
      const result = await transaction(async (conn) => {
        ...
        for (const item of itemRows) {
          ...
          const [batch] = (await conn.execute(
            ...
            [item.batch_no, item.material_id, issue.warehouse_id]
          )) as any;

          if (batch.length > 0) {
            if (parseFloat(batch[0].available_qty) < Number(item.quantity)) {
              throw new Error(
                `物料 ${item.material_name} 批次 ${item.batch_no} 可用库存不足: 可用 ${batch[0].available_qty}, 需发 ${item.quantity}`
              );
            }
            const newQty = parseFloat(batch[0].quantity) - Number(item.quantity);
            const newAvail = parseFloat(batch[0].available_qty) - Number(item.quantity);
            if (newQty <= 0 || newAvail <= 0) {
              ...
              await conn.execute(
                ...
                [Number(item.quantity), Number(item.quantity), batch[0].id]
              );
            }
          } else {
            throw new Error(
              `物料 ${item.material_name} 批次 ${item.batch_no} 不存在，无法委外发料`
            );
          }
        ...
        await conn.execute(
          'UPDATE outsource_issue SET status = 3, update_time = NOW() WHERE id = ?',
          [id]
        );
    ...
    if (fields.length > 0) {
      values.push(id);
      await execute(
        `UPDATE outsource_issue SET ${fields.join(', ')} WHERE id = ? AND deleted = 0`,
        values
      );
    }
