...
export const GET = withPermission(async (request: NextRequest, _userInfo) => {
  ...
  if (supplierName) {
    where += ' AND o.supplier_name LIKE ?';
  ...
  const totalRows = await query('SELECT COUNT(*) as total FROM outsource_order o ' + where, params);
...
export const PUT = withPermission(
  ...
  const ts = await getTranslations('Common');
    ...
    if (fields.length > 0) {
      ...
      values.push(id);
      await execute(
        `UPDATE outsource_order SET ${fields.join(', ')} WHERE id = ? AND deleted = 0`,
        values
      );
    }
