...
export const GET = withPermission(async (request: NextRequest, _userInfo) => {
  ...
  const totalRows = await query('SELECT COUNT(*) as total FROM prd_ink ' + where, params);
...
export const POST = withPermission(
  ...
  const ts = await getTranslations('Common');
    ...
    return successResponse({ id: result.insertId }, ts('k_iqnp74'));
  },
...
export const PUT = withPermission(
  ...
  const ts = await getTranslations('Common');
    ...
    if (ink_type !== undefined) {
      fields.push('ink_type = ?');
    ...
    if (safety_stock !== undefined) {
      fields.push('safety_stock = ?');
      values.push(safety_stock);
    }
    if (shelf_life !== undefined) {
      fields.push('shelf_life = ?');
      values.push(shelf_life);
    }
    if (status !== undefined) {
      fields.push('status = ?');
      values.push(status);
    ...
    if (fields.length > 0) {
      values.push(id);
      await execute(
        'UPDATE prd_ink SET ' + fields.join(', ') + ' WHERE id = ? AND deleted = 0',
        values
      );
    }
