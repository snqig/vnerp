...
export const GET = withPermission(async (request: NextRequest, userInfo) => {
  ...
  const totalRows: any = await query(
    'SELECT COUNT(*) as total FROM prd_product_label ' + where,
    params
  );
...
export const PUT = withPermission(async (request: NextRequest, userInfo) => {
  ...
  if (status !== undefined)
    await execute('UPDATE prd_product_label SET status = ? WHERE id = ? AND deleted = 0', [
      status,
      id,
    ]);
  if (print_count !== undefined)
    await execute(
      'UPDATE prd_product_label SET print_count = ?, print_time = NOW() WHERE id = ? AND deleted = 0',
      [print_count, id]
    );
  if (remark !== undefined)
