...
export const GET = withPermission(async (request: NextRequest, _userInfo) => {
  ...
  const equipmentRows = await query(
    `SELECT 
      workshop_id as workshop,
      equipment_type,
      COUNT(*) as equipment_count,
      SUM(capacity_per_hour) as total_capacity_per_hour
    FROM eqp_equipment
    WHERE deleted = 0 AND current_status = 1
    GROUP BY workshop_id, equipment_type
    ORDER BY workshop_id`
  );

  // 2. 获取排程占用情况
  let scheduleFilter = '';
  const scheduleParams: SqlValue[] = [];
  ...
  // 3. 按设备统计占用
  const equipmentUsageRows = await query(
    `SELECT 
      e.id as equipment_id,
      e.equipment_name,
      e.equipment_type,
      e.workshop_id as workshop,
      e.capacity_per_hour,
    ...
    WHERE e.deleted = 0 AND e.current_status = 1
    GROUP BY e.id, e.equipment_name, e.equipment_type, e.workshop_id, e.capacity_per_hour
    ORDER BY e.workshop_id, e.equipment_type`,
