...
export const GET = withPermission(async (request: NextRequest, _userInfo) => {
  ...
  if (!workOrderId) {
    return successResponse([], ts('k_12bmioo'));
  }

  const details = await getWorkOrderColorSequencesDetail(Number(workOrderId));
  return successResponse(details, ts('k_17722lj'));
});
...
export const POST = withPermission(
  ...
  const ts = await getTranslations('Common');
    ...
    if (!workOrderId || !printArea || !planQty) {
      return successResponse(null, ts('k_fifqlw'));
    }
    ...
    return successResponse(result, ts('k_1jkyo9'));
  },
