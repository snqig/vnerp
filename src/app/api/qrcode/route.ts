...
export const GET = withPermission(async (request: NextRequest, _userInfo) => {
  ...
  if (keyword) {
    where +=
      ' AND (q.qr_code LIKE ? OR q.ref_no LIKE ? OR q.material_name LIKE ? OR q.batch_no LIKE ?)';
    const like = `%${keyword}%`;
    params.push(like, like, like, like);
  }

  const totalRows = await query(`SELECT COUNT(*) as total FROM qrcode_record q ${where}`, params);
  ...
  const rows = await query(
    `SELECT q.*, b.available_qty AS batch_available_qty, b.quantity AS batch_total_qty, b.status AS batch_status
     ...
     LEFT JOIN inv_inventory_batch b ON q.batch_no COLLATE utf8mb4_0900_ai_ci = b.batch_no AND b.deleted = 0
