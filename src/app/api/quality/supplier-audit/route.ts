...
export const GET = withPermission(async (request: NextRequest, userInfo) => {
  ...
  const totalRows: any = await query(
    'SELECT COUNT(*) as total FROM qms_supplier_audit ' + where,
    params
  );
  const total = totalRows[0]?.total || 0;
  const rows: any = await query(
    'SELECT * FROM qms_supplier_audit ' + where + ' ORDER BY create_time DESC LIMIT ? OFFSET ?',
...
export const PUT = withPermission(async (request: NextRequest, userInfo) => {
  ...
  if (audit_scope !== undefined) {
    fields.push('audit_scope = ?');
    values.push(audit_scope);
  }
  if (quality_system_score !== undefined) {
    fields.push('quality_system_score = ?');
    values.push(quality_system_score);
  }
  if (delivery_score !== undefined) {
    fields.push('delivery_score = ?');
    values.push(delivery_score);
  }
  if (price_score !== undefined) {
    fields.push('price_score = ?');
    values.push(price_score);
  }
  if (service_score !== undefined) {
    fields.push('service_score = ?');
    values.push(service_score);
  }
  ...
  if (audit_result !== undefined) {
    fields.push('audit_result = ?');
    values.push(audit_result);
  }
  if (improvement_items !== undefined) {
    fields.push('improvement_items = ?');
    values.push(improvement_items);
  }
