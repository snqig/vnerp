    ...
    return successResponse(
      {
        list: result,
        summary: {
          totalWorkOrders: result.reduce((sum: number, r: DbRow) => sum + r.workOrderCount, 0),
          totalStandardCost: result.reduce((sum: number, r: DbRow) => sum + r.standardCost, 0),
          totalActualCost: result.reduce((sum: number, r: DbRow) => sum + r.actualCost, 0),
          totalVariance: result.reduce((sum: number, r: DbRow) => sum + r.costVariance, 0),
          avgVarianceRate:
            result.length > 0
              ? Math.round(
                  result.reduce((sum: number, r: DbRow) => sum + r.costVarianceRate, 0) /
                    result.length
                )
              : 0,
        },
