...
export const GET = withErrorHandler(async (request: NextRequest) => {
  ...
  if (id) {
    const rc = await queryOne('SELECT * FROM sal_reconciliation WHERE id = ? AND deleted = 0', [
      parseInt(id),
    ]);
    if (!rc) return commonErrors.notFound('对账单不存在');
    const details = await query(
      'SELECT * FROM sal_reconciliation_detail WHERE reconciliation_id = ?',
      [parseInt(id)]
    );
...
export const PUT = withErrorHandler(async (request: NextRequest) => {
  ...
  const existing = await queryOne(
    'SELECT id, status FROM sal_reconciliation WHERE id = ? AND deleted = 0',
    [body.id]
  );
  ...
  if (body.received_amount !== undefined) {
    const rc = (await queryOne(
      'SELECT net_amount, discount_amount FROM sal_reconciliation WHERE id = ?',
      [body.id]
    )) as any;
    if (rc) {
      ...
      const discountAmount =
        body.discount_amount !== undefined
          ...
          : parseFloat(rc.discount_amount) || 0;
...
export const DELETE = withErrorHandler(async (request: NextRequest) => {
  ...
  await execute('DELETE FROM sal_reconciliation_detail WHERE reconciliation_id = ?', [
    parseInt(id),
  ]);
