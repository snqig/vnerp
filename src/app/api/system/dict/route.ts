...
export const GET = withPermission(
  async (request: NextRequest, _userInfo: UserInfo) => {
    ...
    // 获取所有字典类型
    const countRows = await query('SELECT COUNT(*) as total FROM sys_dict_type WHERE deleted = 0');
    ...
    for (const type of types) {
      ...
      const itemsWithType = items.map((it: DbRow) => ({ ...it, dict_type: type.dict_type }));
