...
export const GET = withPermission(async (request: NextRequest) => {
  ...
  for (const log of logRows) {
    const entry: any = {
      ...
      operation_type_label: OPERATION_TYPE_LABELS[log.operation_type] || `类型${log.operation_type}`,
    ...
    try {
      ...
      } else if (log.business_type === 'stocktaking' || log.business_type === 'stocktaking_order') {
        const [order]: any = await query(
  ...
  const totalIn = logRows
    .filter((l: any) => l.operation_type === 1)
    .reduce((sum: number, l: any) => sum + parseFloat(l.operation_qty), 0);
  const totalOut = logRows
    .filter((l: any) => l.operation_type === 2)
    .reduce((sum: number, l: any) => sum + parseFloat(l.operation_qty), 0);
