...
export const GET = withPermission(
  ...
  const ts = await getTranslations('Common');
    ...
    if (!resolvedMaterialId && materialCode) {
      ...
      if (mrows.length > 0) {
        resolvedMaterialId = mrows[0].id;
        materialInfo = {
          materialName: mrows[0].material_name,
          specification: mrows[0].specification,
          unit: mrows[0].unit,
        };
      }
    }
    ...
    const totalAvailable = (batches as DbRow[]).reduce(
    ...
    return await transaction(async (conn) => {
      ...
      for (const item of items) {
        ...
        await appendInventoryLog(conn, {
          ...
          remark: `FIFO出库确认-批次${item.batch_no}`,
        ...
        await recomputeInventorySummary(conn, item.material_id, order.warehouse_id);
      }
