...
export const GET = withPermission(async (request: NextRequest, _userInfo) => {
  ...
  let sql = `
    SELECT
      ...
      o.total_amount as baseTotalAmount,
      'CNY' as baseCurrency,
  ...
  if (result.data.length > 0) {
    const orderIds = result.data.map((o: DbRow) => o.id);
    ...
    for (const order of result.data as DbRow[]) {
...
export const POST = withPermission(
  async (request: NextRequest, userInfo) => {
    ...
    try {
      ...
      return errorResponse(`创建出库单失败: ${(e as Error).message}`, 500, 500);
    }
