...
export const GET = withPermission(async (request: NextRequest) => {
  ...
  for (const row of rows) {
    const items = await query('SELECT * FROM inv_production_inbound_item WHERE inbound_id = ?', [
      row.id,
    ]);
    row.items = items;
  }
...
export const PUT = withPermission(async (request: NextRequest) => {
  ...
  if (action === 'post') {
    const result = await transaction(async (conn) => {
      ...
      // T-INV-3 (方案A)：库存写入已移交给 FinishOrderInventoryHandler（监听
      // prod.finish.approved，由下方 FinishOrderApprovedEvent 触发）。本路由不再
      // 直写 inv_inventory / inv_inventory_transaction，避免双写导致库存虚增。
      // 此处仅完成单据状态流转，库存由事件驱动统一处理。
    ...
    if (inboundInfo) {
      const prodItems = await query(
        'SELECT * FROM inv_production_inbound_item WHERE inbound_id = ?',
        [id]
      );

      for (const item of prodItems as DbRow[]) {
      ...
      await logOperation({
        ...
        oper_result: `生产入库 ${inboundInfo.inbound_no} 过账成功，已生成 ${(prodItems as DbRow[]).length} 个成品二维码`,
      });
    }
