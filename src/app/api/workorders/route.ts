  ...
  async (request: NextRequest) => {
    ...
    if (id) {
      ...
      return successResponse({
        ...workOrder,
        items,
        bom_info: (bomInfo as DbRow[])[0] || null,
      });
    }
    ...
    if (order_no) {
      ...
      return successResponse({
        list: workOrders,
        total: (workOrders as DbRow[]).length,
      });
    }
    ...
    return successResponse({
      list: workOrders,
      total: (countResult as DbRow[])[0]?.total || (workOrders as DbRow[]).length,
      page,
      pageSize,
    });
  },
...
export const PUT = withPermission(
  async (request: NextRequest) => {
    ...
    const result = await transaction(async (connection) => {
      ...
      if (bom_id !== undefined && bom_id !== null && bom_id !== '') {
        updateFields.push('bom_id = ?');
        updateParams.push(parseInt(String(bom_id)) || null);
      }
      ...
      return { id: woKey, status, priority, plan_start_date, plan_end_date };
    });
...
export const DELETE = withPermission(
  async (request: NextRequest) => {
    ...
    const deletedWorkOrderNo = await transaction(async (connection) => {
      ...
      // 仅当已发生实际物料领用/完工入库时才禁止删除，避免误删造成业务数据丢失。
      const [consumeRows] = await connection.execute(
        `SELECT COALESCE(picked_qty,0) + COALESCE(finished_qty,0) AS consumed FROM prod_work_order WHERE id = ?`,
        [workOrder.id]
      );
