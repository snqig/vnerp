...
interface Item { id: number; plate_code: string; plate_name: string; plate_type: number; mesh_count: string; size_spec: string; product_name: string; max_use_count: number; used_count: number; remaining_count: number; status: number; }
...
export default function ScreenPlatePage() {
  ...
  const fetchData = async () => { try { const params = new URLSearchParams({ page: String(page), pageSize: '20', plateCode: searchCode, plateName: searchName }); const res = await fetch('/api/prepress/screen-plate?' + params); const result = await res.json(); if (result.success) { setList(result.data.list || []); setTotal(result.data.total || 0); } } catch (e) { console.error(e); } };
  useEffect(() => { fetchData(); }, [page]);

  const handleSave = async () => { try { const method = editItem.id ? 'PUT' : 'POST'; const res = await fetch('/api/prepress/screen-plate', { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(editItem) }); const result = await res.json(); if (result.success) { toast({ title: editItem.id ? '更新成功' : '创建成功' }); setShowDialog(false); fetchData(); } else { toast({ title: '失败', description: result.message, variant: 'destructive' }); } } catch (e) { toast({ title: '失败', variant: 'destructive' }); } };
  const handleDelete = async (id: number) => { if (!confirm('确定删除？')) return; try { const res = await fetch('/api/prepress/screen-plate?id=' + id, { method: 'DELETE' }); const result = await res.json(); if (result.success) { toast({ title: '删除成功' }); fetchData(); } } catch (e) { toast({ title: '失败', variant: 'destructive' }); } };
  ...
  return (
    <MainLayout>
      <div className="p-6 space-y-6">
        ...
        <Dialog open={showDialog} onOpenChange={setShowDialog}><DialogContent className="max-w-lg" resizable><DialogHeader><DialogTitle>{editItem.id ? '编辑网版' : '新增网版'}</DialogTitle></DialogHeader><div className="grid grid-cols-2 gap-4"><div><Label>网版编码</Label><Input value={editItem.plate_code || ''} onChange={e => setEditItem({ ...editItem, plate_code: e.target.value })} /></div><div><Label>网版名称</Label><Input value={editItem.plate_name || ''} onChange={e => setEditItem({ ...editItem, plate_name: e.target.value })} /></div><div><Label>类型</Label><Select value={String(editItem.plate_type || 1)} onValueChange={v => setEditItem({ ...editItem, plate_type: Number(v) })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="1">丝网版</SelectItem><SelectItem value="2">胶印版</SelectItem><SelectItem value="3">柔版</SelectItem><SelectItem value="4">凹版</SelectItem></SelectContent></Select></div><div><Label>目数</Label><Input value={editItem.mesh_count || ''} onChange={e => setEditItem({ ...editItem, mesh_count: e.target.value })} /></div><div><Label>尺寸规格</Label><Input value={editItem.size_spec || ''} onChange={e => setEditItem({ ...editItem, size_spec: e.target.value })} /></div><div><Label>产品名称</Label><Input value={editItem.product_name || ''} onChange={e => setEditItem({ ...editItem, product_name: e.target.value })} /></div><div><Label>最大使用次数</Label><Input type="number" value={editItem.max_use_count ?? ''} onChange={e => setEditItem({ ...editItem, max_use_count: Number(e.target.value) })} /></div></div><DialogFooter><Button variant="outline" onClick={() => setShowDialog(false)}>取消</Button><Button onClick={handleSave}>保存</Button></DialogFooter></DialogContent></Dialog>
      </div>
    </MainLayout>
  );
}
