...
export default function UnqualifiedPage() {
  ...
  const handleSave = async () => { try { const method = editItem.id ? 'PUT' : 'POST'; const res = await fetch('/api/quality/unqualified', { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(editItem) }); const result = await res.json(); if (result.success) { toast({ title: editItem.id ? '更新成功' : '创建成功' }); setShowDialog(false); fetchData(); } else { toast({ title: '失败', description: result.message, variant: 'destructive' }); } } catch (e) { toast({ title: '失败', variant: 'destructive' }); } };
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2 items-center">
            ...
            <TableExportToolbar
              ...
              onPrint={() => {}}
              onExportPDF={() => exportTableToPDF(displayList, '不合格品处理报告', exportColumns, '不合格品处理报告')}
              onExportXLS={() => exportTableToXLS(displayList, '不合格品处理报告', exportColumns)}
              onExportWORD={() => exportTableToWORD(displayList, '不合格品处理报告', exportColumns, '不合格品处理报告')}
            />
          </div>
        </div>
        <Card><CardContent className="p-0">
          <Table><TableHeader><TableRow>
            <TableHead className="w-12"><Checkbox checked={selectedIds.length === displayList.length && displayList.length > 0} onCheckedChange={() => setSelectedIds(selectedIds.length === displayList.length ? [] : displayList.map(i => i.id))} /></TableHead>
            <TableHead className="text-xs w-12 text-center">序号</TableHead>
            <SortableTableHeader field="handle_no" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}><span className="text-xs">处理单号</span></SortableTableHeader>
