...
const statusMap = { 1: { label: '待入库', variant: 'outline' as const }, 2: { label: '已入库', variant: 'default' as const }, 3: { label: '已取消', variant: 'destructive' as const } };
const qcMap = { 0: { label: '未检', variant: 'secondary' as const }, 1: { label: '合格', variant: 'default' as const }, 2: { label: '不合格', variant: 'destructive' as const } };
...
export default function ProductionInboundPage() {
  ...
  const handleSave = async () => { try { const res = await fetch('/api/warehouse/production-inbound', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(editItem) }); const result = await res.json(); if (result.success) { toast({ title: '创建成功' }); setShowDialog(false); refresh(); } else { toast({ title: '失败', description: result.message, variant: 'destructive' }); } } catch (e) { toast({ title: '失败', variant: 'destructive' }); } };
  const handleStatusChange = async (id: number, status: number) => { try { const res = await fetch('/api/warehouse/production-inbound', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, status }) }); const result = await res.json(); if (result.success) { toast({ title: '更新成功' }); refresh(); } } catch (e) { toast({ title: '失败', variant: 'destructive' }); } };
  const handleDelete = async (id: number) => { if (!confirm('确定删除？')) return; try { const res = await fetch('/api/warehouse/production-inbound?id=' + id, { method: 'DELETE' }); const result = await res.json(); if (result.success) { toast({ title: '删除成功' }); refresh(); } } catch (e) { toast({ title: '失败', variant: 'destructive' }); } };
    ...
    <MainLayout>
      <div className="p-6 space-y-6">
        <PageHeader
          ...
          actions={
            <>
              <div className="flex items-center gap-2"><Input placeholder="搜索单号" value={search} onChange={e => setSearch(e.target.value)} className="w-36 h-8 text-sm" /><Button size="sm" variant="outline" onClick={refresh}><Search className="h-3 w-3" /></Button></div>
