...
import { Plus, Search, Edit, Trash2, QrCode, ClipboardList, CheckCircle, XCircle, Eye } from 'lucide-react';
...
export default function StocktakingPage() {
  ...
  const handleSave = async () => {
    try {
      ...
      const result = await res.json();
      if (result.success) {
        toast({ title: '盘点单生成成功', description: `已锁定库存，共 ${result.data.item_count} 项待盘点` });
        setShowDialog(false);
        fetchData();
      } else {
  ...
  const executeScan = async () => {
    ...
    try {
      const res = await fetch(`/api/warehouse/stocktaking/${currentCheckId}/scan`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          qr_code: scanQrCode,
  ...
  const openDetailDialog = async (check: InventoryCheck) => {
    try {
      const res = await fetch(`/api/warehouse/stocktaking/${check.id}/items`);
      const result = await res.json();
      if (result.success) {
    ...
    <MainLayout>
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" onClick={() => { setEditItem({}); setShowDialog(true); }}>
              <Plus className="h-3 w-3 mr-1" />新增盘点
            </Button>
          </div>
        </div>
          ...
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                {list.map(item => {
                  ...
                  return (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="text-xs text-center font-mono">¥{item.diff_amount?.toFixed(2)}</TableCell>
                      <TableCell><Badge variant={st.variant} className="text-xs">{st.label}</Badge></TableCell>
                      <TableCell>
                        <div className="flex gap-1">
                          {item.status === 1 && (
                            <Button size="sm" variant="ghost" className="h-6 text-xs px-2" onClick={() => openScanDialog(item)}>
                              <QrCode className="h-3 w-3 mr-1" />扫码
                            </Button>
                          )}
                          ...
                          {item.status === 1 && (
                            <Button size="sm" variant="ghost" className="h-6 text-xs px-2" onClick={() => handleAction(item.id, 'submit')}>
                              提交
                            </Button>
                          )}
                          {item.status === 2 && (
                            <Button size="sm" variant="ghost" className="h-6 text-xs px-2" onClick={() => setShowApproveDialog(true)}>
                              审批
                            </Button>
                          )}
                ...
                {list.length === 0 && (
                  <TableRow><TableCell colSpan={9} className="text-center text-muted-foreground py-8">暂无盘点记录</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </CardContent>
          ...
          <DialogContent className="max-w-lg" resizable>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select value={String(editItem.warehouse_id || '')} onValueChange={v => setEditItem({ ...editItem, warehouse_id: Number(v) })}>
              ...
              <div>
                <Label>盘点类型</Label>
                <Select value={String(editItem.type || 1)} onValueChange={v => setEditItem({ ...editItem, type: Number(v) })}>
              ...
              <div>
                <Label>盘点人</Label>
                <UserSelect value={editItem.checker_id ? String(editItem.checker_id) : ''} onChange={v => setEditItem({ ...editItem, checker_id: Number(v) })} />
              </div>
        ...
        <Dialog open={showScanDialog} onOpenChange={setShowScanDialog}>
          <DialogContent className="max-w-lg" resizable>
            <DialogHeader><DialogTitle>扫码盘点</DialogTitle></DialogHeader>
            <div className="space-y-4">
              <div>
                <Label>二维码 <span className="text-red-500">*</span></Label>
                <Input
                  value={scanQrCode}
              ...
              <div>
                <Label>实际数量 <span className="text-red-500">*</span></Label>
                <Input
                  type="number"
                  value={scanQuantity}
              ...
              {scanResult && (
                <div className="border rounded p-3 bg-muted/30 space-y-2">
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div><span className="text-muted-foreground">物料名称：</span>{scanResult.material_name}</div>
                    <div><span className="text-muted-foreground">批次号：</span>{scanResult.batch_no || '-'}</div>
                    <div><span className="text-muted-foreground">类型：</span>{SPLIT_FLAG_MAP[scanResult.split_flag] || '整料'}</div>
                    <div><span className="text-muted-foreground">库位：</span>{scanResult.warehouse_location || '-'}</div>
                    <div><span className="text-muted-foreground">账面数量：</span>{scanResult.book_quantity}</div>
                    <div><span className="text-muted-foreground">实际数量：</span>{scanResult.actual_quantity}</div>
                    <div className={`col-span-2 font-bold ${scanResult.difference !== 0 ? 'text-red-600' : 'text-green-600'}`}>
                      差异：{scanResult.difference > 0 ? '+' : ''}{scanResult.difference}
                    </div>
                  </div>
                </div>
              )}
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowScanDialog(false)}>关闭</Button>
              <Button onClick={executeScan}><QrCode className="h-4 w-4 mr-1" />确认盘点</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={showDetailDialog} onOpenChange={setShowDetailDialog}>
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-auto" resizable>
            <DialogHeader><DialogTitle>盘点明细</DialogTitle></DialogHeader>
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {detailItems.map(item => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="text-xs">{SPLIT_FLAG_MAP[item.split_flag] || '整料'}</TableCell>
                    <TableCell className="text-xs text-center">{item.book_quantity}</TableCell>
                    <TableCell className="text-xs text-center">{item.actual_quantity || '-'}</TableCell>
                    <TableCell className={`text-xs text-center font-bold ${item.difference !== 0 ? 'text-red-600' : ''}`}>
        ...
        <Dialog open={showApproveDialog} onOpenChange={setShowApproveDialog}>
          <DialogContent className="max-w-md" resizable>
            <DialogHeader><DialogTitle>审批盘点结果</DialogTitle></DialogHeader>
            <div className="space-y-4">
              <UserSelect value="" onChange={v => {}} label="审批人" />
              <div className="flex gap-2">
                <Button className="flex-1" onClick={() => handleAction(list.find(i => i.status === 2)?.id || 0, 'approve', { approver_id: 1 })}>
                  <CheckCircle className="h-4 w-4 mr-1" />通过
                </Button>
                <Button variant="destructive" className="flex-1" onClick={() => handleAction(list.find(i => i.status === 2)?.id || 0, 'reject')}>
