...
import { InboundQrInvalidationHandler } from '@/application/handlers/InboundQrInvalidationHandler';
...
import { PurchaseInboundReversalHandler } from '@/application/handlers/PurchaseInboundReversalHandler';
...
export class EventRegistry {
  ...
  static initialize(): void {
    ...
    eventBus.subscribe(
      'inbound.unapproved',
      new IdempotentHandler(new PurchaseInboundReversalHandler())
    );
    eventBus.subscribe('inbound.unapproved', new IdempotentHandler(new InboundQrInvalidationHandler()));
    ...
    eventBus.subscribe('inbound.cancelled', new IdempotentHandler(new InboundQrInvalidationHandler()));
    ...
    // 出库单事件
    // 注意：outbound.approved 不再在此注册库存扣减/应收 handler。
    // 出库扣减与应收生成已由 warehouse/outbound/confirm 路由在单事务内完成（唯一权威扣减点），
    // 若此处再注册 OutboundInventoryHandler/OutboundReceivableHandler 会在事件被触发时
    // 与路由重复扣库存、重复生成应收。原 handler 文件保留为历史参考，但不再订阅。
    ...
    // T-INV-4 (D3): 报工审核不再扣料，仅保留审计日志。
    // 材料扣减唯一权威为领料事件 workorder.material_issued（见上方订阅）。
