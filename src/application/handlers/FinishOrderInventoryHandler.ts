...
export class FinishOrderInventoryHandler {
  async handle(event: DomainEvent): Promise<void> {
    ...
    await transaction(async (conn) => {
      ...
      const batchNo = `FN${finishOrderId}`;
      ...
      // T-INV-3 (方案A)：本 handler 为完工入库库存唯一权威。
      // 1) 建/加批次（成品，按 finishOrderId 确定性批次号；幂等块内仅首次执行）
      const [batchRows] = await conn.execute<RowDataPacket[]>(
        'SELECT id, quantity FROM inv_inventory_batch WHERE batch_no = ? AND material_id = ? AND warehouse_id = ? AND deleted = 0 FOR UPDATE',
      ...
      if (batchRows.length > 0) {
        await conn.execute(
          ...
          [qualifiedQty, qualifiedQty, batchRows[0].id]
        );
      } else {
        await conn.execute(
      ...
      // 2) 派生汇总（批次 SUM → inv_inventory），保证账实一致、无漂移
