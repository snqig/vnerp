...
export default function PrepressReportsContent() {
  ...
  const [dieData, setDieData] = useState<Record<string, any> | null>(null);
  const [inkData, setInkData] = useState<Record<string, any> | null>(null);
  const [surplusData, setSurplusData] = useState<Record<string, any> | null>(null);
            ...
            <>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                  ...
                  <CardContent className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      <RechartsPieChart>
                        <Pie
                          data={(dieData.statusDistribution || []).map((s: DbRow) => ({
                            name: s.die_status,
                            value: s.count,
                          }))}
                          dataKey="value"
                          ...
                          label
                        >
                          {(dieData.statusDistribution || []).map((_: DbRow, i: number) => (
                ...
                <Card>
                  ...
                  <CardContent className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                        data={(dieData.lifeDistribution || []).map((s: DbRow) => ({
                          name: s.life_level,
                          count: s.count,
                        }))}
                      >
              ...
              <Card>
                ...
                <CardContent className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart
                      data={(dieData.usageTrend || []).map((s: DbRow) => ({
                        date: s.date,
                ...
                <Card>
                  ...
                  <CardContent>
                    <Table>
                      ...
                      <TableBody>
                        {(dieData.topUsedDies || []).map((d: Record<string, any>, i: number) => (
                          <TableRow key={i}>
            ...
            <>
              ...
              <Card>
                ...
                <CardContent className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart
                      data={(inkData.dailyTrend || []).map((s: DbRow) => ({
                        date: s.date,
              ...
              <Card>
                ...
                <CardContent>
                  <Table>
                    ...
                    <TableBody>
                      {(inkData.topWorkOrders || []).map((d: Record<string, any>, i: number) => (
                        <TableRow key={i}>
                ...
                <KpiCard
                  ...
                  value={formatNum(
                    surplusData.openingStatus?.reduce(
                      (a: number, s: DbRow) => a + Number(s.count),
                      0
                    ) || 0
                  )}
                  icon={<Package className="h-4 w-4 text-muted-foreground" />}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                  ...
                  <CardContent className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                        data={(surplusData.monthlyReuseTrend || []).map((s: DbRow) => ({
                          month: s.month,
                          rate:
                            Number(s.total) > 0
                ...
                <Card>
                  ...
                  <CardContent className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      <RechartsPieChart>
                        <Pie
                          data={(surplusData.openingStatus || []).map((s: DbRow) => ({
                            name:
                              s.status === 1
                                ? t('overview')
                                : s.status === 2
                          ...
                          label
                        >
                          {(surplusData.openingStatus || []).map((_: DbRow, i: number) => (
                            <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />
                          ))}
                        </Pie>
                        <Tooltip />
                ...
                <CardContent>
                  {(surplusData.expiredWarning || []).length === 0 ? (
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {(surplusData.expiredWarning || []).map(
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                  ...
                  <CardContent className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                        data={(toolData.typeStatusStats || []).map((s: DbRow) => ({
                ...
                <Card>
                  ...
                  <CardContent className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                        data={(toolData.productCostSummary || []).map((s: DbRow) => ({
                          type: s.tool_type === 1 ? t('toolDieCount') : t('toolPlateCount'),
              ...
              <Card>
                ...
                <CardContent>
                  <Table>
                    ...
                    <TableBody>
                      {(toolData.screenPlateDetail || []).map(
                        (d: Record<string, any>, i: number) => (
                          <TableRow key={i}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                  ...
                  <CardContent className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                        data={(sampleData.monthlyTrend || []).map((s: DbRow) => ({
                          month: s.month,
                          count: Number(s.count),
                ...
                <Card>
                  ...
                  <CardContent className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      <RechartsPieChart>
                        <Pie
                          data={(sampleData.costBreakdown || []).map((s: DbRow) => ({
                            name: s.cost_range,
                          ...
                          label
                        >
                          {(sampleData.costBreakdown || []).map((_: DbRow, i: number) => (
                            <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />
                          ))}
                        </Pie>
                        <Tooltip />
              ...
              <Card>
                ...
                <CardContent>
                  <Table>
                    ...
                    <TableBody>
                      {(sampleData.topByCustomer || []).map((d: Record<string, any>, i: number) => (
                        <TableRow key={i}>
