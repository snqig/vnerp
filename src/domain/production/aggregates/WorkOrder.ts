...
export class WorkOrder {
  ...
  static create(props: WorkOrderProps): WorkOrder {
    ...
    const wo = new WorkOrder(
      props.id, props.workOrderNo || '', WorkOrderStatusVO.draft(),
      props.productId, props.productName || '', props.productCode || '',
      props.plannedQty, props.completedQty || 0,
      props.processId, props.processName, props.warehouseId || 1,
      props.plannedStartDate, props.plannedEndDate,
      undefined, undefined, props.remark || '',
      props.createBy, materialReqs,
      props.createTime, props.updateTime
    );

    if (wo.id) {
      wo._domainEvents.push(new WorkOrderCreatedEvent({
        workOrderId: wo.id, workOrderNo: wo.workOrderNo,
        productId: wo.productId, productName: wo.productName, plannedQty: wo.plannedQty,
      }));
    }
    return wo;
  }
  ...
  static reconstitute(props: WorkOrderProps): WorkOrder {
    ...
    return new WorkOrder(
      props.id, props.workOrderNo || '', WorkOrderStatusVO.from(props.status || 'draft'),
      props.productId, props.productName || '', props.productCode || '',
      props.plannedQty, props.completedQty || 0,
      props.processId, props.processName, props.warehouseId || 1,
      props.plannedStartDate, props.plannedEndDate,
      props.actualStartDate, props.actualEndDate, props.remark || '',
      props.createBy, materialReqs,
      props.createTime, props.updateTime
    );
  }

  get status(): WorkOrderStatusVO { return this._status; }
  get plannedQty(): number { return this._plannedQty; }
  get completedQty(): number { return this._completedQty; }
  get actualStartDate(): string | undefined { return this._actualStartDate; }
  get actualEndDate(): string | undefined { return this._actualEndDate; }
  get materialRequirements(): MaterialRequirement[] { return [...this._materialRequirements]; }

  release(): void {
    if (!this._status.canRelease()) throw new DomainError(`当前状态"${this._status.label()}"不允许下达`);
    this._status = this._status.transitionTo('released');
    this._domainEvents.push(new WorkOrderReleasedEvent({
      workOrderId: this.id!, workOrderNo: this.workOrderNo,
      materialRequirements: this._materialRequirements.map(mr => ({
        materialId: mr.materialId, materialCode: mr.materialCode, requiredQty: mr.requiredQty,
      })),
    }));
  }

  start(): void {
    if (!this._status.canStart()) throw new DomainError(`当前状态"${this._status.label()}"不允许开工`);
    this._status = this._status.transitionTo('in_progress');
    this._actualStartDate = new Date().toISOString().slice(0, 19).replace('T', ' ');
    this._domainEvents.push(new WorkOrderStartedEvent({
      workOrderId: this.id!, workOrderNo: this.workOrderNo,
    }));
  }
  ...
  issueMaterials(
    ...
    const issuedItems: Array<{ materialId: number; materialCode: string; materialName: string; quantity: number; batchNo: string; warehouseId: number }> = [];
    ...
    for (const issue of issues) {
      ...
      issuedItems.push({
        materialId: mr.materialId, materialCode: mr.materialCode, materialName: mr.materialName,
        quantity: issue.quantity, batchNo: issue.batchNo, warehouseId: issue.warehouseId,
      });
    }

    this._domainEvents.push(new WorkOrderMaterialIssuedEvent({
      workOrderId: this.id!, workOrderNo: this.workOrderNo, issuedItems,
    }));
  }

  complete(completedQty: number, warehouseId: number): void {
    if (!this._status.canComplete()) throw new DomainError(`当前状态"${this._status.label()}"不允许完工`);
    this._completedQty += completedQty;
    if (this._completedQty > this._plannedQty) {
      throw new DomainError(`完工数量超限: 计划${this._plannedQty}, 已完工${this._completedQty - completedQty}, 本次${completedQty}`);
    }
    this._status = this._status.transitionTo('completed');
    this._actualEndDate = new Date().toISOString().slice(0, 19).replace('T', ' ');

    this._domainEvents.push(new WorkOrderCompletedEvent({
      workOrderId: this.id!, workOrderNo: this.workOrderNo,
      productId: this.productId, productName: this.productName,
      completedQty, warehouseId,
    }));
  }

  close(): void {
    this._status = this._status.transitionTo('closed');
    this._domainEvents.push(new WorkOrderClosedEvent({ workOrderId: this.id!, workOrderNo: this.workOrderNo }));
  }
