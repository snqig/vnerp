...
export class MaterialRequirement {
  ...
  static create(props: MaterialRequirementProps): MaterialRequirement {
    ...
    return new MaterialRequirement(
      props.id, props.materialId, props.materialCode || '', props.materialName || '',
      props.specification || '', props.unit || '件', props.requiredQty, 0, 0,
      props.warehouseId || 1
    );
  }
  ...
  static reconstitute(props: MaterialRequirementProps): MaterialRequirement {
    return new MaterialRequirement(
      props.id, props.materialId, props.materialCode || '', props.materialName || '',
      props.specification || '', props.unit || '件', props.requiredQty,
      props.issuedQty || 0, props.returnedQty || 0, props.warehouseId || 1
    );
  }

  get requiredQty(): number { return this._requiredQty; }
  get issuedQty(): number { return this._issuedQty; }
  get returnedQty(): number { return this._returnedQty; }

  get remainingQty(): number { return this._requiredQty - this._issuedQty + this._returnedQty; }
  get isFullyIssued(): boolean { return this._issuedQty >= this._requiredQty; }
  ...
  issue(quantity: number): void {
    ...
    if (newIssuedQty > this._requiredQty) {
      throw new DomainError(`领料超限: 需求${this._requiredQty}, 已领${this._issuedQty}, 本次${quantity}`);
    }
  ...
  returnMaterial(quantity: number): void {
    ...
    if (quantity > this._issuedQty - this._returnedQty) {
      throw new DomainError(`退料超限: 已领${this._issuedQty}, 已退${this._returnedQty}, 本次${quantity}`);
    }
