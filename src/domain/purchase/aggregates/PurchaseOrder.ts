...
export class PurchaseOrder {
  ...
  reverseReceive(lineReceives: Array<{ lineNo: number; quantity: number }>): void {
    ...
    if (allZero) {
      this._status = this._status.transitionTo('approved');
    } else if (this.isFullyReceived) {
      if (this._status.value !== 'completed') {
        this._status = this._status.transitionTo('completed');
      }
    } else if (this._status.value !== 'partially_received') {
      this._status = this._status.transitionTo('partially_received');
    }

    const totalReversedAmount = reversedItems.reduce(
      (sum, item) => sum + item.quantity * item.unitPrice,
      0
    );

    this._domainEvents.push(
      new PurchaseOrderReceiveReversedEvent({
        orderId: this.id!,
        orderNo: this.orderNo,
        supplierId: this.supplierId,
        supplierName: this.supplierName,
  ...
  canReverseReceive(): boolean {
    return this._status.canReverseReceive();
  }
