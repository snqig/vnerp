...
export interface IDomainEventOutboxRepository {
  ...
  /**
   * 列出所有死信事件（status=dead_letter），按创建时间倒序
   * @param limit 单次最大条数
   */
  fetchDeadLetterEvents(limit?: number): Promise<EventOutboxRecord[]>;

  /**
   * 重放单条死信事件：重置回 pending 并清空重试计数与错误信息，立即可被重新消费。
   * 仅对 status=dead_letter 的行生效（WHERE 兜底，避免误重置其他状态）。
   * 调用方须在根因修复后手动触发——重放会重新派发给所有 handler，依赖 handler 幂等。
   * @param id 事件 ID
   * @returns 受影响行数（0 表示 id 不存在或非死信）
   */
  replayDeadLetter(id: number): Promise<number>;

  /**
   * 批量重放所有死信事件：将所有 status=dead_letter 重置回 pending。
   * @returns 重放的事件数量
   */
  replayAllDeadLetters(): Promise<number>;
}
