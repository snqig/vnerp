...
export abstract class BaseRepository<T, F extends BaseFilters = BaseFilters> {
  constructor(
    ...
    protected readonly db: DbExecutor = mysqlDbExecutor
  ) {}
  ...
  async findList(
    ...
    const rows = await this.db.query<Record<string, unknown>>(dataSql, [
      ...params,
      pageSize,
      (page - 1) * pageSize,
    ]);
  ...
  async existsByCode(code: string, excludeId?: number): Promise<boolean> {
    ...
    const result = await this.db.queryOne<{ cnt: number }>(sql, params);
  ...
  protected async queryPaginatedRaw(
    ...
    const rows = await this.db.query<Record<string, unknown>>(`${dataSql} LIMIT ? OFFSET ?`, [
      ...params,
      pageSize,
      (page - 1) * pageSize,
    ]);
