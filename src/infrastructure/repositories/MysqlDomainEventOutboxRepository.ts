...
export class MysqlDomainEventOutboxRepository implements IDomainEventOutboxRepository {
   ...
   */
  async fetchDeadLetterEvents(limit: number = 100): Promise<EventOutboxRecord[]> {
    const rows = await query(
      `SELECT id, event_type, aggregate_type, aggregate_id, payload, status,
              retry_count, error_message, next_execute_at, create_time, processed_at
       FROM domain_event_outbox
  ...
  /**
   * 重放单条死信事件：status→pending、retry_count→0、error_message→NULL、next_execute_at→NOW()
   * 仅对 status=dead_letter 生效（WHERE 兜底）
   */
  async replayDeadLetter(id: number): Promise<number> {
    ...
    const replayed = result.affectedRows;
    if (replayed > 0) {
      secureLog('info', 'Dead letter event replayed', { eventId: id });
    }
    return replayed;
  }
  ...
  async replayAllDeadLetters(): Promise<number> {
    ...
    const replayed = result.affectedRows;
    if (replayed > 0) {
      secureLog('info', 'All dead letter events replayed', { count: replayed });
    }
    return replayed;
  }
