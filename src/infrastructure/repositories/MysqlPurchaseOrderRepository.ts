...
export class MysqlPurchaseOrderRepository implements IPurchaseOrderRepository {
  ...
  async findByStatus(
    ...
    if (status) {
      // 支持逗号分隔多状态：status=30,40 → o.status IN (30,40)；'all' 表示不过滤
      const statusList = status
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean);
      if (statusList.length > 0 && !statusList.includes('all')) {
        const dbCodes: number[] = [];
        for (const s of statusList) {
          const code = /^\d+$/.test(s)
            ? PurchaseOrderStatus.fromDbCode(Number(s)).toDbCode()
            : PurchaseOrderStatus.from(s).toDbCode();
          if (!dbCodes.includes(code)) dbCodes.push(code);
        }
        const condition = ` AND o.status IN (${dbCodes.map(() => '?').join(',')})`;
        sql += condition;
        countSql += condition;
        dbCodes.forEach((c) => params.push(c));
      }
    }
