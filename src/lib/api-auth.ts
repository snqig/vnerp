...
export function withAuthAndErrorHandler(
  ...
  return async (request: NextRequest, context?: RouteHandlerContext): Promise<NextResponse> => {
    try {
      ...
      // AppError 携带标准 HTTP 状态码（如冲突 409），按其声明的状态码返回，避免统一吞成 500
      if (error instanceof AppError) {
        return errorResponse(error.message, error.statusCode, error.statusCode);
      }
      // 数据库唯一约束冲突（如重复提交入库/出库单、重复明细行）：转为 409 业务冲突提示
