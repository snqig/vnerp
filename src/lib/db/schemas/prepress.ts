...
export const dcprintInkFormulaVersion = mysqlTable(
  ...
  (table) => ({
    ...
    fk_dcprintInkColor_colorId: foreignKey({
      name: 'fk_formula_version_color',
      columns: [table.colorId],
      foreignColumns: [dcprintInkColor.id],
    })
      .onDelete('restrict')
      .onUpdate('cascade'),
  })
...
export const dcprintInkFormulaItem = mysqlTable(
  ...
  (table) => ({
    ...
    fk_dcprintInkFormulaVersion_versionId: foreignKey({
      name: 'fk_formula_item_version',
      columns: [table.versionId],
      foreignColumns: [dcprintInkFormulaVersion.id],
    })
      .onDelete('cascade')
      .onUpdate('cascade'),
  })
