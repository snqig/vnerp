...
export const sysUser = mysqlTable(
  ...
  (table) => ({
    ...
    statusIdx: index('idx_user_status').on(table.status, table.deleted),
      fk_sysDepartment_departmentId: foreignKey({
      name: 'fk_sys_user_department',
      columns: [table.departmentId],
      foreignColumns: [sysDepartment.id],
    })
      .onDelete('set null')
      .onUpdate('cascade'),
  })
