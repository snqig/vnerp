...
export function handleError(error: unknown, req?: NextRequest): NextResponse {
  ...
  if (error instanceof AppError) {
    ...
    // 生产环境绝不向客户端泄露底层异常信息（可能包含 SQL、表结构等敏感细节）
    rawMessage = error.message;
    if (isUniqueViolation(error)) {
      // 唯一约束冲突：转为业务冲突提示（409），不抛 500
      appError = AppError.conflict(mapUniqueErrorToMessage(error));
    } else {
  ...
  if (!isProd) {
    ...
    // 生产环境：剔除可能含原始数据库错误的字段（如 DatabaseError.originalMessage）
    const safe = sanitizeClientDetails(appError.details);
    if (safe && Object.keys(safe).length > 0) {
      response.details = safe;
    }
  }

  logError(appError, req, rawMessage);

  return NextResponse.json(response, { status: appError.statusCode });
}

/**
 * 剔除详情中可能泄露内部信息的敏感字段（如 originalMessage）。
 * 保留业务层有意下发的字段级校验错误等安全信息。
 */
function sanitizeClientDetails(details: ErrorDetails): ErrorDetails {
...
function logError(error: AppError, req?: NextRequest, rawMessage?: string) {
  const logData = {
