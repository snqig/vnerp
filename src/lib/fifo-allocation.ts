...
function calculateAllocation(
  ...
  const result: FIFOAllocationResult = {
    ...
    material_code: batches.length > 0 ? String(batches[0].material_code) : '',
    material_name: batches.length > 0 ? String(batches[0].material_name) : '',
  ...
  const totalAvailableDecimal = batches.reduce(
    (sum: Decimal, b: DbRow) => sum.plus(new Decimal(String(b.available_qty))),
    new Decimal(0)
  );
  ...
  batches.forEach((b, idx) => {
    logger.debug(
      `[FIFO]   batch[${idx}]: id=${String(b.id)}, batch_no=${String(b.batch_no)}, available_qty=${String(b.available_qty)}, inbound_date=${String(b.inbound_date)}, expire_date=${String(b.expire_date)}, opened_at=${String(b.opened_at)}, split_flag=${String(b.split_flag)}, version=${String(b.version)}`
    );
  });
  ...
  for (const batch of batches) {
    ...
    const availableQtyDecimal = new Decimal(String(batch.available_qty));
    const allocateQtyDecimal = Decimal.min(remainingDecimal, availableQtyDecimal);

    logger.debug(
      `[FIFO]   allocating: batch_no=${String(batch.batch_no)}, available=${availableQtyDecimal.toNumber()}, remaining=${remainingDecimal.toNumber()}, allocate=${allocateQtyDecimal.toNumber()}`
    );

    result.allocations.push({
      batch_id: Number(batch.id),
      batch_no: String(batch.batch_no),
      material_id: Number(batch.material_id),
      material_code: String(batch.material_code),
      material_name: String(batch.material_name),
      allocate_qty: allocateQtyDecimal.toNumber(),
      available_qty_before: availableQtyDecimal.toNumber(),
      unit_cost: new Decimal(String(batch.unit_price || 0)).toNumber(),
      inbound_date: batch.inbound_date ? String(batch.inbound_date) : '',
      expire_date: batch.expire_date ? String(batch.expire_date) : undefined,
      opened_at: batch.opened_at ? String(batch.opened_at) : undefined,
      version: batch.version !== undefined ? Number(batch.version) : undefined,
    });
...
export async function planFIFOAllocation(
  ...
  const [batches]: DbResult = await conn.query(sql, params);
...
export async function allocateFIFO(
  ...
  const [batches]: DbResult = await conn.query(lockedSql, params);
...
export async function checkShortageAndWarn(
  ...
  const safetyRows = await query<DbRow>(
  ...
  const safetyStock = parseFloat(String(safety_stock)) || 0;
  const reorderPoint = parseFloat(String(reorder_point)) || 0;

  const invRows = await query<DbRow>(
  ...
  const availableQty = parseFloat(String(invRows[0]?.total_available)) || 0;
  // 缺料量 = 需求量 - 可用量（至少为0）
...
): Promise<{ deductionDetails: DbRow[]; totalCost: number; attempts: number }> {
  logger.debug(
...
async function executeFIFODeductionInternal(
  ...
  for (const alloc of allocation.allocations) {
    ...
    const [updateResult]: DbResult = await conn.execute(
    ...
    const result = updateResult as unknown as DbResultSetHeader;
    logger.debug(
      `[FIFO]   batch ${alloc.batch_no} UPDATE result - affectedRows: ${result.affectedRows}`
    );

    if (result.affectedRows === 0) {
      // 乐观锁冲突：查询当前批次实际状态，用于生成详细错误信息
      const [currentBatch]: DbResult = await conn.query(
    ...
    const [currentInv]: DbResult = await conn.query(
      'SELECT quantity FROM inv_inventory WHERE material_id = ? AND warehouse_id = ? AND deleted = 0',
      [alloc.material_id, params.warehouseId]
    );
    const beforeQty = currentInv.length > 0 ? parseFloat(String(currentInv[0].quantity)) : 0;
    ...
    await appendInventoryLog(conn, {
      ...
      operationType: 2,
      operationQty: alloc.allocate_qty,
      beforeQty,
      afterQty,
      businessType: params.sourceType,
      businessNo: params.sourceNo,
      remark: `FIFO出库-批次${alloc.batch_no}`,
...
export async function executeSpecifiedBatchDeduction(
...
): Promise<{ deductionDetail: DbRow; totalCost: number }> {
  const [batch]: DbResult = await conn.query(
  ...
  const availableQtyDecimal = new Decimal(String(batchData.available_qty));
  ...
  const unitCostDecimal = new Decimal(String(batchData.unit_price || 0));
  ...
  await appendInventoryLog(conn, {
    ...
    operationType: 2,
    operationQty: params.requiredQty,
    beforeQty,
    afterQty,
    businessType: params.sourceType,
    businessNo: params.sourceNo,
    remark: `指定批次出库-${params.batchNo}`,
    operatorId: params.operatorId,
  });
