...
export async function recordReceipt(
  ...
  try {
    const result = await transaction(async (conn) => {
      // 1. 查询应收单
      const [recRows]: DbResult = await conn.execute(`SELECT * FROM fin_receivable WHERE id = ?`, [
        receivableId,
      ]);
      ...
      await conn.execute(
        `INSERT INTO fin_receipt_record (
          ...
          receipt_date, receipt_method, remark, deleted, create_time
        ) VALUES (?, ?, ?, ?, ?, ?, ?, 0, NOW())`,
        [
          receiptNo,
          receivableId,
  ...
  try {
    ...
    const result: Loose = await execute(
      `INSERT INTO fin_payable (
        payable_no, source_type, source_no,
...
export async function recordPayment(
  ...
  try {
    const result = await transaction(async (conn) => {
      const [payRows]: DbResult = await conn.execute(`SELECT * FROM fin_payable WHERE id = ?`, [
        payableId,
      ]);
