...
type LodopType = {
  ...
  GET_VALUE: (name: string, ...args: unknown[]) => string | number;
};
...
async function ensureLodop(): Promise<boolean> {
  ...
  if ((window as unknown as { CLODOP?: LodopType }).CLODOP) return true;
    ...
    _loadPromise = new Promise<void>((resolve, reject) => {
      ...
      script.onload = () => {
        const checkExist = setInterval(() => {
          if ((window as unknown as { CLODOP?: LodopType }).CLODOP) {
  ...
  try {
    await _loadPromise;
    return !!(window as unknown as { CLODOP?: LodopType }).CLODOP;
  } catch {
    return false;
  }
}
...
export function getLodop(): LodopType | null {
  ...
  const LODOP = (window as unknown as { CLODOP?: LodopType }).CLODOP;
