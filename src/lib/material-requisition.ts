...
import { planFIFOBatches, enforceFIFO, checkWholeMaterial } from '@/lib/warehouse-core';
  ...
  try {
    ...
    const result = await transaction(async (conn) => {
      ...
      for (const bomItem of bomRows) {
        ...
        // FIFO推荐批次（只读规划，仅用于展示/推荐，实际扣减走 fifo-allocation 加锁路径）
        const fifoResult = await planFIFOBatches(bomItem.material_id, plannedQty);
  ...
  try {
    ...
    const result = await transaction(async (conn) => {
      for (const item of items) {
        ...
        // 4. 扣减库存（复用外层事务连接：不自行 begin/commit，
        //    外层回滚时库存扣减一并回滚，杜绝"单据失败但库存已扣"）
...
export async function confirmReturn(
  ...
  try {
    ...
    // 整体事务：全部退料入库 + 状态更新要么一起成功，要么一起回滚。
    // adjustInventory 复用事务连接，杜绝"库存已加但单据未确认"的不一致。
    await transaction(async (conn) => {
      for (const item of itemRows) {
        ...
        if (!inventoryResult.success) {
          throw new Error(`库存回写失败: ${inventoryResult.message}`);
        }
      }
