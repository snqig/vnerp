...
const DEFAULT_CAPACITY_PER_HOUR = 50;
...
export function calculatePriorityScore(
  ...
  const rawScore = urgencyScore + customerScore + ageScore;

  // 策略A：缺料不参与评分；调整归一化范围
  const minPossible = 25; // low + 无客户 + 无等待
  const maxPossible = 140; // urgent + 有客户 + 最长等待
...
export function suggestScheduleDates(params: {
  ...
  // 产能模型：工期(工作日) = max(1, ceil(数量 / (产能/小时 × 日工时)))
  const dailyCapacity = (capacityPerHour ?? DEFAULT_CAPACITY_PER_HOUR) * workingHoursPerDay;
  const estimatedDays = Math.max(1, Math.ceil(workOrder.quantity / dailyCapacity));
...
export async function checkMaterialAvailability(params: {
  ...
  const workOrders = await query<DbRow>(
  ...
  const bomLines = await query<DbRow>(
  ...
  for (const line of bomLines) {
    ...
    const inventoryResult = await query<DbRow>(
...
export async function getCapacityLoad(params: {
  ...
  // 按工时计算负载百分比：load% = (总占用工时 / 总可用工时) × 100
  // 总可用工时 = 工单数 × 日工时（假设每台设备日可用 = DEFAULT_WORKING_HOURS_PER_DAY）
  // 注意：当工单跨设备时实际负载 = 100% × 并发度，此处按单设备视角计算
  for (const load of loadMap.values()) {
    const totalAvailableHours = load.totalOrders * DEFAULT_WORKING_HOURS_PER_DAY;
    load.loadPercentage = totalAvailableHours > 0
      ? Math.round((load.totalWorkHours / totalAvailableHours) * 100)
      : 0;
  }
