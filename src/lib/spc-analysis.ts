...
export async function getSPCDataFromDB(
  ...
  `;
  const params: SqlValue[] = [materialId, startDate, endDate];
  ...
  const measurements: { timestamp: string; value: number }[] = rows.map((row: DbRow) => ({
    timestamp: row.create_time instanceof Date ? row.create_time.toISOString() : String(row.create_time),
