...
describe('验证工具测试', () => {
  describe('Validator.validateValue - 单值验证', () => {
    ...
    it('应该验证枚举值', () => {
      const result = Validator.validateValue('invalid', {
        field: 'status',
    ...
    it('应该通过有效枚举值', () => {
      const result = Validator.validateValue('active', {
        field: 'status',
        enum: ['active', 'inactive'],
      });
      expect(result).toBeNull();
  ...
  describe('Validator.validate - 对象验证', () => {
    it('应该验证对象中的多个字段', () => {
      const result = Validator.validate({ username: '', age: 'invalid' }, [
    ...
    it('应该通过有效对象验证', () => {
      const result = Validator.validate({ username: 'john', age: 25 }, [
        { field: 'username', required: true },
