...
describe('端到端流程：销售→MRP→生产→库存→应收', () => {
  ...
  describe('Step 2: 工单领料 → 库存扣减', () => {
    it('WorkOrderMaterialIssuedHandler 应扣减 inv_inventory 和 inv_inventory_batch', async () => {
      ...
      mockConn.execute
        ...
        .mockResolvedValueOnce([{ affectedRows: 1 }] as any)
        .mockResolvedValueOnce([{ affectedRows: 1 }] as any) // recomputeInventorySummary: 汇总已存在→更新后即返回
      ...
      const txnCall = mockConn.execute.mock.calls.find(
        (c) => typeof c[0] === 'string' && c[0].includes('INSERT INTO inv_inventory_transaction')
      );
      expect(txnCall).toBeDefined();
      expect((txnCall![1] as unknown[])).toContain('out');
    });
  ...
  describe('Step 3: 工单完工 → 成品入库', () => {
    it('WorkOrderCompletedHandler 应增加 inv_inventory 和创建 inv_inventory_batch', async () => {
      ...
      mockConn.execute
        ...
        .mockResolvedValueOnce([{ affectedRows: 1 }] as any)
        .mockResolvedValueOnce([{ insertId: 1 }] as any)
        .mockResolvedValueOnce([{ affectedRows: 1 }] as any) // recomputeInventorySummary
        .mockResolvedValueOnce([{ insertId: 1 }] as any);
      ...
      expect(compTxn).toBeDefined();
      expect((compTxn![1] as unknown[])).toContain('in');
      expect((compTxn![1] as unknown[])).toContain('workorder_completion');
    });
  ...
  describe('Step 4: 销售出库 → 库存扣减 + 应收凭证', () => {
    it('SalesShippedHandler 应扣减库存并记录出库流水', async () => {
      ...
      const shipTxn = mockConn.execute.mock.calls.find(
        (c) => typeof c[0] === 'string' && c[0].includes('INSERT INTO inv_inventory_transaction')
      );
      expect(shipTxn).toBeDefined();
      expect((shipTxn![1] as unknown[])).toContain('out');
      expect((shipTxn![1] as unknown[])).toContain('sales');
    });
