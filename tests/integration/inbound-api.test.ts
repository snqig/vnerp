...
// 入库单创建前会做物料分类强制校验（category.require_on_business）。
// API 路由测试关注路由/参数/错误映射，不依赖真实分类库，故 mock 该模块。
vi.mock('@/lib/category-validation', () => ({
  checkMaterialsCategorized: vi.fn().mockResolvedValue({
    blocked: false,
    uncategorized: [],
    message: null,
  }),
...
describe('入库 API 集成测试', () => {
  ...
  describe('POST /api/warehouse/inbound - 创建入库单', () => {
    ...
    it('ERR-IN 物料未归类且 require_on_business 阻断时返回 400', async () => {
      vi.mocked(checkMaterialsCategorized).mockResolvedValueOnce({
        blocked: true,
        uncategorized: [{ id: 101, material_code: 'M001', material_name: '材料1' }],
        message: '以下物料尚未设置物料分类：M001(材料1)。请先在「基础数据 → 物料分类」中归类。',
      });
      ...
      expect(data.success).toBe(false);
      expect(data.message).toContain('物料分类');
      expect(mockInboundService.createOrder).not.toHaveBeenCalled();
    });
  });
