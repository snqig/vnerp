...
describe('PurchaseOrder Aggregate', () => {
  ...
  describe('receive() 超收容差 (overReceiptTolerance)', () => {
    it('超过 orderQty*(1+tolerance/100) 应抛 DomainError', () => {
      const order = PurchaseOrder.reconstitute(
        buildValidProps({ id: 1, status: 'approved', overReceiptTolerance: 5 })
      );
      // 行1 orderQty=100，容差5% → 上限105
      expect(() =>
        order.receive([{ lineNo: 1, quantity: 106, batchNo: 'B1', warehouseId: 1 }])
      ).toThrow(DomainError);
    });
    ...
    it('等于容差上限(105)应允许', () => {
      ...
      expect(() =>
        order.receive([{ lineNo: 1, quantity: 105, batchNo: 'B1', warehouseId: 1 }])
      ).not.toThrow();
      expect(order.lines[0].receivedQty).toBe(105);
    });

    it('未指定容差时默认 0，上限=订购量', () => {
      const order = PurchaseOrder.reconstitute(
        buildValidProps({ id: 1, status: 'approved', overReceiptTolerance: undefined })
      );
      expect(() =>
        order.receive([{ lineNo: 1, quantity: 101, batchNo: 'B1', warehouseId: 1 }])
      ).toThrow(DomainError);
      expect(() =>
        order.receive([{ lineNo: 1, quantity: 100, batchNo: 'B1', warehouseId: 1 }])
      ).not.toThrow();
    });
  });

  describe('reverseReceive() 回补', () => {
    it('回补全部已收应回退状态到 approved', () => {
      const order = PurchaseOrder.reconstitute(
        buildValidProps({ id: 1, status: 'approved', overReceiptTolerance: 5 })
      );
      order.receive([{ lineNo: 1, quantity: 50, batchNo: 'B1', warehouseId: 1 }]);
      expect(order.status.value).toBe('partially_received');

      order.reverseReceive([{ lineNo: 1, quantity: 50 }]);
      expect(order.lines[0].receivedQty).toBe(0);
      expect(order.totalReceivedQty).toBe(0);
      expect(order.status.value).toBe('approved');
    });

    it('回补超过已收数应抛 DomainError', () => {
      const order = PurchaseOrder.reconstitute(
        buildValidProps({ id: 1, status: 'approved', overReceiptTolerance: 5 })
      );
      order.receive([{ lineNo: 1, quantity: 50, batchNo: 'B1', warehouseId: 1 }]);
      expect(() => order.reverseReceive([{ lineNo: 1, quantity: 60 }])).toThrow(DomainError);
    });
    ...
    it('已完成单回补部分应转为部分入库', () => {
      ...
      order.reverseReceive([{ lineNo: 1, quantity: 40 }]);
      expect(order.lines[0].receivedQty).toBe(60);
      expect(order.status.value).toBe('partially_received');
    });
    ...
    it('无收货的已审核单不允许回补', () => {
      ...
      expect(() => order.reverseReceive([{ lineNo: 1, quantity: 10 }])).toThrow(DomainError);
    });
  });
