...
describe('8.2 InboundItem 实体', () => {
  describe('create() 工厂', () => {
    ...
    it('materialId 为 0 合法（自由录入物料，未关联主数据）', () => {
      const item = InboundItem.create(makeProps({ materialId: 0 }));
      expect(item.materialId).toBe(0);
    });

    it('materialId 缺失或为负抛错', () => {
      expect(() => InboundItem.create(makeProps({ materialId: undefined as unknown as number }))).toThrow(
        /物料ID不能为空/
      );
      expect(() => InboundItem.create(makeProps({ materialId: -1 }))).toThrow(/物料ID不能为空/);
    });
