...
describe('阶段 3：事件消费 — Handler 通过 EventBus 消费事件并执行库存写操作', () => {
  it('EventBus.publish 触发 FinishOrderInventoryHandler 执行库存入库', async () => {
    ...
    // T-INV-3（方案A）：handler 为完工入库库存唯一权威。
    // 采用 SQL 路由 mock 覆盖完整调用链，避免脆弱的顺序 once 队列：
    // 1. SELECT prod_work_order → 工单产品信息
    // 2. INSERT IGNORE inv_inventory_transaction → affectedRows=1（首次处理，幂等记录）
    // 3. SELECT inv_inventory_batch → 无现有批次
    // 4. INSERT inv_inventory_batch → 新建批次（成品库存权威写入）
    // 5. recomputeInventorySummary（real）→ 读取批次合计并派生 inv_inventory
    mocks.mockConn.execute.mockImplementation(async (sql: string) => {
      const s = String(sql);
      const u = s.toUpperCase();
      // 注意：recompute 的派生 UPDATE 内含嵌套 SELECT ... FROM inv_inventory_batch，
      // 因此必须先判定 INSERT/UPDATE（写操作），再判定 SELECT，否则会被 SELECT 分支误匹配。
      if (u.includes('SELECT') && s.includes('prod_work_order')) {
        return mockRows([{ product_id: 50, product_code: 'P001', product_name: '彩色包装盒' }]);
      }
      if (u.includes('INSERT') && u.includes('IGNORE')) {
        return mockResult(1); // 首次处理
      }
      if ((u.includes('INSERT') || u.includes('UPDATE')) && s.includes('inv_inventory')) {
        // 批次写（inv_inventory_batch）与 recompute 派生 UPDATE（inv_inventory）均走此分支
        return mockResult(1);
      }
      if (u.includes('SELECT') && s.includes('inv_inventory')) {
        return mockRows([]); // 任何库存相关 SELECT → 空结果
      }
      return mockResult(1);
    });
    ...
    // 验证执行了批次写操作（INSERT inv_inventory_batch，非 IGNORE、非 transaction 表）
    // —— T-INV-3 方案A：批次为库存权威来源
    const batchWriteCall = mocks.mockConn.execute.mock.calls.find((c) => {
    ...
    // 验证派生汇总链路被触发：recompute 执行了针对 inv_inventory 的派生 UPDATE
    // （其 SQL 内含 inv_inventory_batch 子查询，故按全大写 "UPDATE INV_INVENTORY" 精确匹配）
    const summaryWriteCall = mocks.mockConn.execute.mock.calls.find((c) =>
      String(c[0]).toUpperCase().includes('UPDATE INV_INVENTORY')
    );
