...
describe('warehouse-core', () => {
  ...
  describe('planFIFOBatches', () => {
    it('FIFO 启用：余料优先排序分配', async () => {
      ...
      const result = await planFIFOBatches(101, 80);
    ...
    it('FIFO 禁用：按入库时间排序', async () => {
      ...
      const result = await planFIFOBatches(101, 50);
    ...
    it('带 warehouseId 时 SQL 包含仓库过滤', async () => {
      ...
      await planFIFOBatches(101, 30, 1);
    ...
    it('库存不足时返回部分分配结果', async () => {
      ...
      const result = await planFIFOBatches(101, 100);
